...
export const registry: ComponentMeta[] = [
  ...
  {
    tag: 'cg-grid', name: 'Grid', category: 'layout',
    description: 'CSS-grid layout primitive with declarative columns, gap, and alignment. Use `min-column` for responsive auto-fill wrapping. Shares cg-stack\'s gap scale.',
    props: [
      { name: 'columns', type: 'number', default: '1', description: 'Number of equal columns' },
      { name: 'min-column', type: 'string', default: '""', description: 'Responsive: auto-fill columns of at least this min-width (overrides columns)' },
      { name: 'gap', type: '"none" | "xs" | "sm" | "md" | "lg" | "xl" | "2xl"', default: '"md"', description: 'Gap between cells' },
      { name: 'align', type: '"start" | "center" | "end" | "stretch"', default: '"stretch"', description: 'Align items (block axis)' },
      { name: 'justify', type: '"start" | "center" | "end" | "stretch"', default: '"stretch"', description: 'Justify items (inline axis)' },
      { name: 'full', type: 'boolean', default: 'false', description: 'Stretch to full width' },
    ],
    events: [],
    examples: [
      { label: '3-column grid', html: `<cg-grid columns="3" gap="md"><cg-card>1</cg-card><cg-card>2</cg-card><cg-card>3</cg-card><cg-card>4</cg-card><cg-card>5</cg-card><cg-card>6</cg-card></cg-grid>` },
      { label: 'Responsive auto-fill', html: `<cg-grid min-column="180px" gap="md"><cg-card>A</cg-card><cg-card>B</cg-card><cg-card>C</cg-card><cg-card>D</cg-card></cg-grid>` },
      { label: 'Two columns', html: `<cg-grid columns="2" gap="lg"><cg-card>Left</cg-card><cg-card>Right</cg-card></cg-grid>` },
    ],
    since: 'v0.8.0',
  },
  {
    tag: 'cg-center', name: 'Center', category: 'layout',
    description: 'Centers content on both axes, with an optional max-width for constraining readable content blocks. Use `full` to fill the parent height.',
    props: [
      { name: 'inline', type: 'boolean', default: 'false', description: 'Center inline-axis only, keep block flow' },
      { name: 'max-width', type: 'string', default: '""', description: 'Max-width for the centered content wrapper' },
      { name: 'full', type: 'boolean', default: 'false', description: 'min-height: 100% to enable vertical centering' },
      { name: 'gap', type: '"none" | "xs" | "sm" | "md" | "lg" | "xl" | "2xl"', default: '"none"', description: 'Gap between stacked children' },
    ],
    events: [],
    examples: [
      { label: 'Centered spinner', html: `<cg-center full style="min-height:160px;border:1px dashed var(--cg-color-surface-cards-border);border-radius:8px;"><cg-spinner></cg-spinner></cg-center>` },
      { label: 'Constrained content column', html: `<cg-center max-width="420px" inline gap="sm"><cg-text as="h3">Readable width</cg-text><cg-text color="secondary">This content column is capped so lines stay comfortable to read.</cg-text></cg-center>` },
      { label: 'Empty-state style', html: `<cg-center full gap="sm" style="min-height:200px;"><cg-text as="h4">Nothing here yet</cg-text><cg-button variant="primary">Create one</cg-button></cg-center>` },
    ],
    since: 'v0.8.0',
  },
  {
    tag: 'cg-app-shell', name: 'App Shell', category: 'layout',
    description: 'Top-level application scaffold composing header, sidebar, content, and footer regions with landmark roles and a responsive collapsible sidebar. Compose cg-navbar (header) + cg-sidebar (sidebar) inside it.',
    props: [
      { name: 'sidebar-position', type: '"start" | "end"', default: '"start"', description: 'Which side the sidebar sits on' },
      { name: 'sidebar-width', type: 'string', default: '"260px"', description: 'Sidebar track width' },
      { name: 'sidebar-collapsed', type: 'boolean', default: 'false', description: 'Collapse the sidebar' },
      { name: 'header-height', type: 'string', default: '"56px"', description: 'Header track height' },
      { name: 'sticky-header', type: 'boolean', default: 'true', description: 'Header stays fixed while content scrolls' },
    ],
    events: [{ name: 'cg-app-shell-toggle', detail: '{collapsed: boolean}', description: 'Sidebar collapse toggled' }],
    examples: [
      { label: 'Basic shell', html: `<cg-app-shell style="height:280px;border:1px solid var(--cg-color-surface-base-divider);border-radius:8px;overflow:hidden;"><div slot="header" style="display:flex;align-items:center;padding:0 16px;font-weight:600;">My App</div><div slot="sidebar" style="padding:12px;"><cg-stack gap="xs"><cg-text>Dashboard</cg-text><cg-text>Reports</cg-text><cg-text>Settings</cg-text></cg-stack></div><div style="padding:16px;"><cg-text>Main content area</cg-text></div><div slot="footer" style="padding:8px 16px;"><cg-text size="sm" color="secondary">© Cognivo</cg-text></div></cg-app-shell>` },
      { label: 'Collapsed sidebar', html: `<cg-app-shell sidebar-collapsed style="height:240px;border:1px solid var(--cg-color-surface-base-divider);border-radius:8px;overflow:hidden;"><div slot="header" style="display:flex;align-items:center;padding:0 16px;font-weight:600;">Collapsed</div><div slot="sidebar" style="padding:12px;">Nav</div><div style="padding:16px;">Content spans full width</div></cg-app-shell>` },
      { label: 'Sidebar on end', html: `<cg-app-shell sidebar-position="end" style="height:240px;border:1px solid var(--cg-color-surface-base-divider);border-radius:8px;overflow:hidden;"><div slot="header" style="display:flex;align-items:center;padding:0 16px;font-weight:600;">End-aligned</div><div slot="sidebar" style="padding:12px;">Nav</div><div style="padding:16px;">Content</div></cg-app-shell>` },
    ],
    since: 'v0.8.0',
  },
  {
    tag: 'cg-overflow-list', name: 'Overflow List', category: 'layout',
    description: 'Lays out slotted items in a single row and collapses those that don\'t fit into a trailing "more" menu. Responsive, ResizeObserver-driven.',
    props: [
      { name: 'gap', type: '"none" | "xs" | "sm" | "md" | "lg"', default: '"sm"', description: 'Gap between items' },
      { name: 'more-label', type: 'string', default: '"More"', description: 'Accessible label for the overflow trigger' },
      { name: 'min-visible', type: 'number', default: '0', description: 'Always keep at least N items visible' },
    ],
    events: [
      { name: 'cg-overflow-change', detail: '{hiddenCount: number, hiddenIndices: number[]}', description: 'Collapsed set changed' },
      { name: 'cg-overflow-select', detail: '{index: number}', description: 'A hidden item was chosen from the menu' },
    ],
    examples: [
      { label: 'Action row', html: `<cg-overflow-list more-label="More actions" style="max-width:280px;"><cg-button variant="secondary" size="sm">Cut</cg-button><cg-button variant="secondary" size="sm">Copy</cg-button><cg-button variant="secondary" size="sm">Paste</cg-button><cg-button variant="secondary" size="sm">Delete</cg-button><cg-button variant="secondary" size="sm">Duplicate</cg-button></cg-overflow-list>` },
      { label: 'Keep 2 visible', html: `<cg-overflow-list min-visible="2" style="max-width:200px;"><cg-chip>All</cg-chip><cg-chip>Open</cg-chip><cg-chip>Closed</cg-chip><cg-chip>Archived</cg-chip></cg-overflow-list>` },
      { label: 'Wide (no overflow)', html: `<cg-overflow-list style="max-width:600px;"><cg-button size="sm">One</cg-button><cg-button size="sm">Two</cg-button><cg-button size="sm">Three</cg-button></cg-overflow-list>` },
    ],
    since: 'v0.8.0',
  },
  {
    tag: 'cg-lightbox', name: 'Lightbox', category: 'overlays',
    description: 'Fullscreen image viewer with prev/next navigation, keyboard control, focus trap, and scroll lock. Pairs with cg-image-gallery. Set the `images` property in JS.',
    props: [
      { name: 'open', type: 'boolean', default: 'false', description: 'Controls visibility' },
      { name: 'index', type: 'number', default: '0', description: 'Current image index' },
      { name: 'closable', type: 'boolean', default: 'true', description: 'Show close button + allow Escape/backdrop close' },
    ],
    events: [
      { name: 'cg-lightbox-open', detail: 'void', description: 'Opened' },
      { name: 'cg-lightbox-close', detail: 'void', description: 'Closed' },
      { name: 'cg-lightbox-change', detail: '{index: number}', description: 'Navigated to another image' },
    ],
    examples: [
      { label: 'Open a gallery', html: `<cg-button variant="primary" onclick="this.nextElementSibling.open=true">Open lightbox</cg-button><cg-lightbox></cg-lightbox>`, setup: (el) => { const lb = el.querySelector('cg-lightbox'); if (lb) lb.images = [{ src: 'https://picsum.photos/id/1015/900/600', alt: 'River', caption: 'A river in the mountains' }, { src: 'https://picsum.photos/id/1016/900/600', alt: 'Canyon', caption: 'Desert canyon' }, { src: 'https://picsum.photos/id/1018/900/600', alt: 'Forest', caption: 'Misty forest' }]; } },
    ],
    since: 'v0.8.0',
  },
  {
    ...
    props: [
      ...
      { name: 'color', type: '"default" | "secondary" | "muted" | "accent" | "success" | "warning" | "danger" | "inherit"', default: '"default"', description: 'Text color — `default` → `secondary` → `muted` form a 3-level hierarchy' },
    ],
    events: [],
    examples: [
      ...
      { label: 'Hierarchy', html: `<cg-stack gap="xs"><cg-text color="default" weight="semibold">Default — primary headings &amp; body</cg-text><cg-text color="secondary">Secondary — subheadings &amp; supporting copy</cg-text><cg-text color="muted">Muted — captions, timestamps, labels</cg-text></cg-stack>` },
      { label: 'Semantic colors', html: `<cg-stack gap="xs"><cg-text color="accent" weight="medium">Accent — brand highlight</cg-text><cg-text color="success">Success message</cg-text><cg-text color="warning">Warning message</cg-text><cg-text color="danger">Error message</cg-text></cg-stack>` },
    ],
    since: 'v0.1.0',
  },
  {
  ...
  },
  {
    ...
    description: 'Static semantic status label — display only, no interaction. For clickable / removable / selectable pills, use cg-chip. For toolbar press-state buttons, use cg-toggle.',
    props: [
    ...
    examples: [
      ...
      { label: 'With dot indicator', html: `<cg-stack direction="row" gap="sm"><cg-badge variant="success" label="Active" dot></cg-badge><cg-badge variant="danger" label="Error"></cg-badge><cg-badge variant="info" label="Online" dot></cg-badge></cg-stack>` },
    ],
    since: 'v0.1.0',
  },
  {
  ...
  {
    tag: 'cg-callout', name: 'Callout', category: 'feedback',
  ...
  {
    tag: 'cg-icon', name: 'Icon', category: 'data-display',
  ...
  {
    tag: 'cg-follow-up', name: 'Follow Up', category: 'ai-display',
  ...
  {
    tag: 'cg-accordion', name: 'Accordion', category: 'data-display',
  ...
  },
  {
    ...
    description: 'Syntax-highlighted code display with filename tab, language chip, copy button, line numbers, and optional collapse for long blocks.',
    props: [
      { name: 'code', type: 'string', description: 'Code content' },
      { name: 'language', type: 'string', description: 'Language label (also used for syntax highlighting heuristics)' },
      { name: 'filename', type: 'string', description: 'Filename shown in the header tab' },
      { name: 'line-numbers', type: 'boolean', default: 'false', description: 'Show line numbers gutter' },
      { name: 'wrap', type: 'boolean', default: 'false', description: 'Soft-wrap long lines instead of horizontal scroll' },
      { name: 'collapsible', type: 'boolean', default: 'false', description: 'Collapse blocks longer than 15 lines with a "Show all" affordance' },
      { name: 'rounded', type: '"none" | "sm" | "md" | "lg"', default: '"lg"', description: 'Border radius variant' },
    ],
    events: [],
    examples: [
      {
        label: 'TypeScript with filename',
        html: `<cg-code-block></cg-code-block>`,
        setup: (el) => {
          const c = el.querySelector('cg-code-block') as any;
          if (c) {
            c.filename = 'greeting.ts';
            c.language = 'typescript';
            c.lineNumbers = true;
            c.code = `import { Cognivo } from '@cognivo/core';\n\nfunction greet(name: string): string {\n  return \`Hello, \${name}!\`;\n}\n\nconsole.log(greet('Cognivo'));`;
          }
        },
      },
      {
        label: 'Minimal (no filename)',
        html: `<cg-code-block></cg-code-block>`,
        setup: (el) => {
          const c = el.querySelector('cg-code-block') as any;
          if (c) {
            c.language = 'bash';
            c.code = `pnpm add @cognivo/components\npnpm dev`;
          }
        },
      },
      {
        label: 'Collapsible long block',
        html: `<cg-code-block collapsible line-numbers></cg-code-block>`,
        setup: (el) => {
          const c = el.querySelector('cg-code-block') as any;
          if (c) {
            c.filename = 'config.ts';
            c.language = 'typescript';
            c.code = Array.from({ length: 24 }, (_, i) => `export const setting${i + 1} = ${i * 7 + 3};`).join('\n');
          }
        },
      },
    ],
  ...
  {
    tag: 'ai-confidence-badge', name: 'AI Confidence Badge', category: 'ai-display',
    description: 'AI model confidence score badge with animated transitions, tooltip, sparkline history, 3 sizes, keyboard accessible. Renamed from `ai-badge` to clarify its purpose vs. the generic `cg-badge` status pill.',
    props: [
    ...
    events: [{ name: 'ai-confidence-badge-click', detail: '{score, level}', description: 'Badge clicked' }],
    examples: [
      { label: 'Confidence levels', html: `<cg-stack direction="row" gap="md"><ai-confidence-badge score="0.95"></ai-confidence-badge><ai-confidence-badge score="0.65"></ai-confidence-badge><ai-confidence-badge score="0.25"></ai-confidence-badge></cg-stack>` },
      { label: 'Large with bar', html: `<ai-confidence-badge score="0.88" size="lg" explanation="Based on 5 data sources"></ai-confidence-badge>` },
    ],
    since: 'v0.1.0',
  },
  {
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    ...
    props: [
      ...
      { name: 'agentRole', type: 'string', default: '"Data Analyst"', description: 'Agent role (attribute: agent-role — role would clobber host ARIA)' },
  ...
  { tag: 'cg-command', name: 'Command', category: 'overlays', description: 'Searchable command palette (Cmd+K-style modal) with combobox pattern, grouped items, leading icons, kbd-pill shortcuts, keyword type-ahead filtering, and full keyboard navigation. Used as the foundation for ai-command-palette. Each CommandItem accepts: id, label, group?, icon? (cg-icon name), shortcut?, keywords?, disabled?.', props: [{name:'open',type:'boolean',default:'false',description:'Open state — when true, the modal palette renders centered over the viewport with a backdrop'},{name:'placeholder',type:'string',default:'"Type a command or search…"',description:'Input placeholder'},{name:'commands',type:'CommandItem[]',default:'[]',description:'Available commands. Each item: { id, label, group?, icon?, shortcut?, keywords?, disabled? }. Items with the same group render together under an uppercase group label.'},{name:'value',type:'string',default:'""',description:'Current search query — bound to the input. Filters by label and keywords.'},{name:'empty-text',type:'string',default:'"No results found."',description:'Empty-state text shown when no commands match the query'},{name:'loading',type:'boolean',default:'false',description:'Loading state — replaces the list with a centered cg-spinner + "Loading…" label. Use while async-fetching commands.'}], events: [{name:'cg-command-select',detail:'{ id: string, command: CommandItem }',description:'A command was selected (click or Enter). Palette closes automatically; input clears.'},{name:'cg-command-input',detail:'{ value: string }',description:'Search query changed — fires on every input event'},{name:'cg-command-open',detail:'none',description:'Palette opened (backdrop + modal both visible)'},{name:'cg-command-close',detail:'none',description:'Palette closed (Escape, backdrop click, or item selection)'}], examples: [{label:'Developer command palette (with icons)',html:`
...
`.trim(),setup:(el)=>{const c=el.querySelector('cg-command') as any;const btn=el.querySelector('#cmd-trigger');if(c){c.commands=[{id:'new-file',label:'New file',group:'File',icon:'document',shortcut:'⌘N'},{id:'open',label:'Open file…',group:'File',icon:'folder',shortcut:'⌘O'},{id:'save',label:'Save',group:'File',icon:'download',shortcut:'⌘S'},{id:'save-as',label:'Save as…',group:'File',shortcut:'⌘⇧S',disabled:true},{id:'undo',label:'Undo',group:'Edit',shortcut:'⌘Z'},{id:'redo',label:'Redo',group:'Edit',shortcut:'⌘⇧Z'},{id:'find',label:'Find',group:'Edit',icon:'minimalistic-magnifer-linear',shortcut:'⌘F',keywords:['search']},{id:'theme',label:'Toggle dark mode',group:'View',icon:'eye',shortcut:'⌘K ⌘T'},{id:'sidebar',label:'Toggle sidebar',group:'View',shortcut:'⌘B'},{id:'docs',label:'Open documentation',group:'Help',icon:'document'},{id:'shortcuts',label:'Keyboard shortcuts',group:'Help',icon:'key',shortcut:'⌘/'}];}btn?.addEventListener('click',()=>{if(c)c.open=true;});}},{label:'Loading state',html:`
<cg-stack direction="column" gap="md">
...
`.trim(),setup:(el)=>{const c=el.querySelector('cg-command') as any;const btn=el.querySelector('#cmd-loading-trigger');btn?.addEventListener('click',()=>{if(c){c.loading=true;c.open=true;setTimeout(()=>{c.commands=[{id:'a',label:'Loaded command',group:'Async'},{id:'b',label:'Another loaded command',group:'Async'}];c.loading=false;},1500);}});}},{label:'Custom empty state',html:`
<cg-stack direction="column" gap="md">
...
`.trim(),setup:(el)=>{const c=el.querySelector('cg-command') as any;const btn=el.querySelector('#cmd-empty-trigger');if(c)c.commands=[{id:'a',label:'Apple',group:'Fruit'},{id:'b',label:'Banana',group:'Fruit'}];btn?.addEventListener('click',()=>{if(c)c.open=true;});}}], since:'v0.7.0' },
  ...
  { tag: 'cg-link', name: 'Link', category: 'actions', description: 'Styled anchor with underline-from-center hover animation, external icon, 4 variants (default/accent/muted/underline) and 3 sizes.', props: [{name:'href',type:'string',description:'Link URL'},{name:'variant',type:'"default" | "accent" | "muted" | "underline"',default:'"default"',description:'Style variant'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Font size'},{name:'external',type:'boolean',default:'false',description:'Opens in new tab + adds external icon'},{name:'disabled',type:'boolean',default:'false',description:'Disabled state'}], events: [{name:'cg-link-click',detail:'none',description:'Anchor clicked (suppressed when disabled)'}], examples: [{label:'Variants',html:'<cg-stack direction="row" gap="md"><cg-link href="#" variant="default">Default</cg-link><cg-link href="#" variant="accent">Accent</cg-link><cg-link href="#" variant="muted">Muted</cg-link><cg-link href="#" variant="underline">Underline</cg-link><cg-link href="#" external>External</cg-link></cg-stack>'},{label:'Sizes',html:'<cg-stack direction="row" gap="md" align="center"><cg-link href="#" size="sm">Small</cg-link><cg-link href="#" size="md">Medium</cg-link><cg-link href="#" size="lg">Large</cg-link></cg-stack>'}], since:'v0.5.0' },
  ...
  { tag: 'cg-toolbar', name: 'Toolbar', category: 'actions', description: 'Grouped-action container implementing the ARIA toolbar roving-tabindex pattern: one tab stop, arrow keys move between actions. Skips disabled items and separators.', props: [{name:'orientation',type:'"horizontal" | "vertical"',default:'"horizontal"',description:'Arrow-key axis and layout direction'},{name:'label',type:'string',default:'""',description:'aria-label for the toolbar landmark'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Gap and padding scale'},{name:'wrap',type:'boolean',default:'false',description:'Allow items to wrap to multiple lines'}], events: [], examples: [{label:'Formatting toolbar',html:'<cg-toolbar label="Text formatting"><cg-button variant="ghost">Bold</cg-button><cg-button variant="ghost">Italic</cg-button><cg-separator orientation="vertical"></cg-separator><cg-button variant="ghost">Link</cg-button></cg-toolbar>'},{label:'Vertical',html:'<cg-toolbar orientation="vertical" label="Tools"><cg-button variant="ghost">Move</cg-button><cg-button variant="ghost">Draw</cg-button><cg-button variant="ghost">Erase</cg-button></cg-toolbar>'},{label:'With disabled action',html:'<cg-toolbar label="Editor"><cg-button variant="ghost">Undo</cg-button><cg-button variant="ghost" disabled>Redo</cg-button><cg-button variant="ghost">Save</cg-button></cg-toolbar>'}], since:'v0.8.0' },
  ...
  { tag: 'cg-rating', name: 'Rating', category: 'forms', description: 'Star rating input with keyboard navigation (arrow keys), half-star precision, hover preview, and clear-on-same-value.', props: [{name:'value',type:'number',default:'0',description:'Current rating'},{name:'max',type:'number',default:'5',description:'Maximum stars'},{name:'precision',type:'0.5 | 1',default:'1',description:'Step precision'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Star size'},{name:'readonly',type:'boolean',default:'false',description:'Read-only mode'},{name:'disabled',type:'boolean',default:'false',description:'Disabled state'}], events: [{name:'cg-rating-change',detail:'{value: number}',description:'Rating changed'}], examples: [{label:'Basic',html:'<cg-rating value="4"></cg-rating>'},{label:'Half-star precision',html:'<cg-rating value="3.5" precision="0.5"></cg-rating>'},{label:'Large readonly',html:'<cg-rating value="5" size="lg" readonly></cg-rating>'}], since:'v0.7.0' },
  { tag: 'cg-tag-input', name: 'Tag Input', category: 'forms', description: 'Chip-based multi-value input. Type + Enter (or comma) to add tags · Backspace on empty input removes the last tag. Pill chips with subtle border + soft hover · screen-reader live announcements · error/success states.', props: [{name:'value',type:'string[]',default:'[]',description:'Current tags'},{name:'label',type:'string',default:'""',description:'Label text'},{name:'placeholder',type:'string',default:'""',description:'Input placeholder'},{name:'helper',type:'string',default:'""',description:'Helper text below the field'},{name:'delimiter',type:'string',default:'","',description:'Delimiter to split tags'},{name:'max',type:'number',default:'0',description:'Maximum tags (0 = unlimited)'},{name:'allow-duplicates',type:'boolean',default:'false',description:'Allow duplicate tags'},{name:'error',type:'boolean',default:'false',description:'Error state'},{name:'success',type:'boolean',default:'false',description:'Success state'},{name:'disabled',type:'boolean',default:'false',description:'Disabled state'},{name:'size',type:'"md" | "lg"',default:'"md"',description:'Field size'}], events: [{name:'cg-tag-add',detail:'{value: string[], tag: string}',description:'Tag added'},{name:'cg-tag-remove',detail:'{value: string[], tag: string}',description:'Tag removed'},{name:'cg-tag-change',detail:'{value: string[]}',description:'Tags changed'}], examples: [{label:'Pre-filled chips',html:'<cg-tag-input label="Skills" placeholder="Type a skill, press Enter…" helper="Hover a chip to see the soft bg shift · click × to remove" style="max-width:480px;"></cg-tag-input>',setup:(el)=>{const t=el.querySelector("cg-tag-input");if(t)t.value=["TypeScript","Lit","Web Components","Design Tokens","Accessibility"];}},{label:'Empty — start typing',html:'<cg-tag-input label="Tags" placeholder="Type and press Enter or comma" helper="Backspace on empty input removes the last tag" style="max-width:480px;"></cg-tag-input>'},{label:'Max 3 (try adding a 4th)',html:'<cg-tag-input label="Top tags" placeholder="Max 3 tags…" max="3" helper="Live region announces the limit" style="max-width:480px;"></cg-tag-input>',setup:(el)=>{const t=el.querySelector("cg-tag-input");if(t)t.value=["one","two"];}},{label:'Error state',html:'<cg-tag-input label="Tags (required)" placeholder="Add a tag…" error helper="At least one tag is required" style="max-width:480px;"></cg-tag-input>'},{label:'Success state',html:'<cg-tag-input label="Tags" placeholder="Add a tag…" success helper="Looks good!" style="max-width:480px;"></cg-tag-input>',setup:(el)=>{const t=el.querySelector("cg-tag-input");if(t)t.value=["frontend","accessible","modern"];}},{label:'Sizes (md / lg)',html:'<div style="display:flex;flex-direction:column;gap:16px;max-width:480px;"><cg-tag-input label="Medium (default)" placeholder="Add a tag…" size="md"></cg-tag-input><cg-tag-input label="Large" placeholder="Add a tag…" size="lg"></cg-tag-input></div>',setup:(el)=>{const inputs=el.querySelectorAll("cg-tag-input");inputs.forEach((t,i)=>{t.value=i===0?["small","medium","large"]:["bigger","chips","yes"];});}},{label:'Email recipients',html:'<cg-tag-input label="To" placeholder="Enter email addresses…" helper="Press Enter or comma after each email" size="lg" style="max-width:480px;"></cg-tag-input>',setup:(el)=>{const t=el.querySelector("cg-tag-input");if(t)t.value=["alice@example.com","bob@team.io"];}},{label:'GitHub-style topics',html:'<cg-tag-input label="Repository topics" placeholder="Add a topic…" max="6" helper="Up to 6 topics" style="max-width:480px;"></cg-tag-input>',setup:(el)=>{const t=el.querySelector("cg-tag-input");if(t)t.value=["typescript","web-components","design-system","ai-native"];}},{label:'Disabled',html:'<cg-tag-input label="Locked tags" disabled style="max-width:480px;"></cg-tag-input>',setup:(el)=>{const t=el.querySelector("cg-tag-input");if(t)t.value=["read-only","tag"];}}], since:'v0.7.0' },
  { tag: 'cg-phone-input', name: 'Phone Input', category: 'forms', description: 'International phone-number input with searchable country selector — flag, dial code, and country name in one composed control. Search by name, ISO code, or +dial-code · preferred countries pinned to top · emits a single E.164 value · keyboard-friendly · live-region announcements on country change.', props: [{name:'value',type:'string',default:'""',description:'Full E.164 number (e.g. +15551234567)'},{name:'country',type:'string',default:'"US"',description:'ISO-3166-1 alpha-2 country code'},{name:'default-country',type:'string',default:'"US"',description:'Initial country when value/country are empty'},{name:'label',type:'string',default:'""',description:'Floating label'},{name:'placeholder',type:'string',default:'""',description:'National-number placeholder (falls back to country format)'},{name:'helper',type:'string',default:'""',description:'Helper text below the field'},{name:'size',type:'"md" | "lg"',default:'"md"',description:'Field size'},{name:'rounded',type:'"none" | "sm" | "md" | "lg"',default:'"lg"',description:'Border radius'},{name:'preferred-countries',type:'string[]',default:'["US","GB","BR","DE","FR","CA","AU","IN","JP","MX"]',description:'ISO codes pinned to top of list'},{name:'only-countries',type:'string[]',default:'[]',description:'Restrict picker to these ISO codes'},{name:'exclude-countries',type:'string[]',default:'[]',description:'Hide these ISO codes from the list'},{name:'national-mode',type:'boolean',default:'false',description:'Hide the dial code in the trigger (still emits E.164)'},{name:'disabled',type:'boolean',default:'false',description:'Disabled state'},{name:'readonly',type:'boolean',default:'false',description:'Readonly state'},{name:'error',type:'boolean',default:'false',description:'Error state'},{name:'success',type:'boolean',default:'false',description:'Success state'},{name:'loading',type:'boolean',default:'false',description:'Loading state — replaces flag with spinner'},{name:'required',type:'boolean',default:'false',description:'Required field'},{name:'name',type:'string',default:'""',description:'Form field name'}], events: [{name:'cg-phone-input-change',detail:'{value, country, dialCode, nationalNumber, valid}',description:'Number or country changed'},{name:'cg-phone-input-country-change',detail:'{country, dialCode}',description:'User picked a different country'}], examples: [{label:'Default (US)',html:'<cg-phone-input label="Phone" helper="We\\u2019ll text a verification code" style="max-width:420px;"></cg-phone-input>'},{label:'Brazil with custom preferred list',html:'<cg-phone-input label="Telefone" default-country="BR" preferred-countries=\'["BR","PT","US","DE"]\' helper="Selecione o pa\\u00eds e digite o n\\u00famero" style="max-width:420px;"></cg-phone-input>'},{label:'Pre-filled E.164 value',html:'<cg-phone-input label="Mobile" value="+447400123456" helper="Hydrates GB + national number from E.164" style="max-width:420px;"></cg-phone-input>'},{label:'Restricted to a few countries',html:'<cg-phone-input label="Phone" only-countries=\'["US","CA","MX"]\' helper="North-America only" style="max-width:420px;"></cg-phone-input>'},{label:'Sizes (md / lg)',html:'<div style="display:flex;flex-direction:column;gap:16px;max-width:420px;"><cg-phone-input label="Medium (default)" size="md"></cg-phone-input><cg-phone-input label="Large" size="lg"></cg-phone-input></div>'},{label:'Error / Success',html:'<div style="display:flex;flex-direction:column;gap:16px;max-width:420px;"><cg-phone-input label="Phone" error helper="Please enter a valid number"></cg-phone-input><cg-phone-input label="Phone" success value="+15551234567" helper="Looks good"></cg-phone-input></div>'},{label:'Disabled',html:'<cg-phone-input label="Phone" disabled value="+15551234567" style="max-width:420px;"></cg-phone-input>'}], since:'v0.8.0' },
  { tag: 'cg-file-input', name: 'File Input', category: 'forms', description: 'File picker with drag-and-drop, paste-from-clipboard, image thumbnails, file-type-aware icons, mobile camera capture, per-file upload status + progress, bulk clear, and screen-reader live announcements. Three sizes, two visual variants.', props: [{name:'label',type:'string',default:'""',description:'Label text'},{name:'accept',type:'string',default:'""',description:'Accepted file types'},{name:'multiple',type:'boolean',default:'false',description:'Allow multiple files'},{name:'max-size',type:'number',default:'0',description:'Max file size in bytes (0 = unlimited)'},{name:'max-files',type:'number',default:'0',description:'Max number of files (0 = unlimited)'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Dropzone size'},{name:'variant',type:'"dropzone" | "compact"',default:'"dropzone"',description:'Visual style — full dropzone or inline button'},{name:'capture',type:'"" | "user" | "environment"',default:'""',description:'Mobile camera capture (front/rear)'},{name:'paste',type:'boolean',default:'false',description:'Accept files pasted from clipboard (Ctrl/Cmd+V)'},{name:'error',type:'boolean',default:'false',description:'Error state'},{name:'disabled',type:'boolean',default:'false',description:'Disabled state'}], events: [{name:'cg-file-change',detail:'{files: File[]}',description:'Files selected'},{name:'cg-file-reject',detail:'{files: File[], reason: string}',description:'File rejected (size/count)'},{name:'cg-file-remove',detail:'{file: File}',description:'File removed'},{name:'cg-file-clear',detail:'{}',description:'All files cleared via bulk action'}], examples: [{label:'Multiple documents (md)',html:'<cg-file-input label="Upload documents" accept=".pdf,.doc,.docx" multiple max-size="5242880" helper="Drop 2+ files to see the bulk Clear-all" style="max-width:480px;"></cg-file-input>'},{label:'Image upload with thumbnails',html:'<cg-file-input label="Photos" accept="image/*" multiple max-size="2097152" helper="Live thumbnails appear after drop" style="max-width:480px;"></cg-file-input>'},{label:'Paste from clipboard',html:'<cg-file-input label="Paste anywhere" paste multiple helper="Copy any image then press ⌘V (Ctrl+V on Windows)" style="max-width:480px;"></cg-file-input>'},{label:'Mobile camera capture',html:'<cg-file-input label="Take a photo" accept="image/*" capture="environment" helper="On mobile, opens the camera directly" style="max-width:480px;"></cg-file-input>'},{label:'Upload progress (call setFileStatus)',html:'<cg-file-input id="demo-status" label="Drop a file then watch the progress" multiple style="max-width:480px;"></cg-file-input>',setup:(el)=>{const input=el.querySelector("cg-file-input");if(!input)return;input.addEventListener("cg-file-change",(e)=>{const files=e.detail.files;files.forEach((file,i)=>{(input).setFileStatus(file,{state:"uploading",progress:0});let p=0;const t=setInterval(()=>{p+=Math.random()*15;if(p>=100){p=100;clearInterval(t);(input).setFileStatus(file,{state:i%3===2?"error":"done",error:i%3===2?"Network timeout":undefined});}else{(input).setFileStatus(file,{state:"uploading",progress:p});}},250+i*120);});});}},{label:'Sizes (sm / md / lg)',html:'<div style="display:flex;flex-direction:column;gap:16px;max-width:480px;"><cg-file-input size="sm" placeholder="Small dropzone"></cg-file-input><cg-file-input size="md" placeholder="Medium (default)"></cg-file-input><cg-file-input size="lg" placeholder="Large dropzone — for hero CTAs"></cg-file-input></div>'},{label:'Compact (inline button)',html:'<div style="display:flex;flex-direction:column;gap:12px;align-items:flex-start;"><cg-file-input variant="compact" multiple></cg-file-input><cg-file-input variant="compact" multiple accept="image/*"></cg-file-input></div>'},{label:'Error / disabled',html:'<div style="display:flex;flex-direction:column;gap:12px;max-width:480px;"><cg-file-input label="With error" error helper="Please upload a valid file"></cg-file-input><cg-file-input label="Disabled" disabled></cg-file-input></div>'}], since:'v0.7.0' },

  // Wave 7: Foundation Completion — Structural
  { tag: 'cg-collapsible', name: 'Collapsible', category: 'data-display', description: 'Single-section disclosure with smooth grid-template-rows animation. Card or flush variants in three sizes. Lighter than accordion.', props: [{name:'open',type:'boolean',default:'false',description:'Open state'},{name:'disabled',type:'boolean',default:'false',description:'Disabled state'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Size — drives padding and font-size'},{name:'variant',type:'"card" | "flush"',default:'"card"',description:'Visual style — bordered card or chrome-less flush'}], events: [{name:'cg-collapsible-toggle',detail:'{open: boolean}',description:'Toggled'}], examples: [{label:'Basic (md card)',html:'<cg-collapsible><span slot="trigger">Show advanced options</span><div>Additional configuration goes here. This content is revealed with a smooth animation when the trigger is clicked.</div></cg-collapsible>'},{label:'Open by default',html:'<cg-collapsible open><span slot="trigger">FAQ: How do I install?</span><p style="margin:0;">Run <code>pnpm add @cognivo/components</code> and import in your app.</p></cg-collapsible>'},{label:'Sizes (sm / md / lg)',html:'<div style="display:flex;flex-direction:column;gap:12px;"><cg-collapsible size="sm"><span slot="trigger">Small</span><div>Compact disclosure for dense layouts.</div></cg-collapsible><cg-collapsible size="md" open><span slot="trigger">Medium (default)</span><div>The standard size for most contexts.</div></cg-collapsible><cg-collapsible size="lg"><span slot="trigger">Large</span><div>Roomy disclosure for primary content sections.</div></cg-collapsible></div>'},{label:'Flush variant',html:'<div style="display:flex;flex-direction:column;gap:8px;"><cg-collapsible variant="flush"><span slot="trigger">What is Cognivo?</span><div>An AI-native component library with cognitive bias analysis built in.</div></cg-collapsible><cg-collapsible variant="flush"><span slot="trigger">How does theming work?</span><div>Override Tier-2 semantic tokens — every component re-skins automatically.</div></cg-collapsible><cg-collapsible variant="flush"><span slot="trigger">Is it accessible?</span><div>Yes — keyboard, ARIA, focus rings, and reduced-motion support across all 183 components.</div></cg-collapsible></div>'},{label:'Disabled',html:'<cg-collapsible disabled><span slot="trigger">Locked section</span><div>Cannot be opened.</div></cg-collapsible>'}], since:'v0.7.0' },
  { tag: 'cg-kbd', name: 'Kbd', category: 'typography', description: 'Keyboard shortcut display chip. Use for showing keyboard combinations like ⌘K or Ctrl+Shift+P.', props: [{name:'keys',type:'string',default:'""',description:'Comma-separated keys (renders with + separators)'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Size'},{name:'variant',type:'"default" | "outline"',default:'"default"',description:'Visual style'}], events: [], examples: [{label:'Single key',html:'<cg-kbd keys="⌘K"></cg-kbd>'},{label:'Multi-key',html:'<cg-kbd keys="⇧,⌘,P"></cg-kbd>'},{label:'Slot content',html:'<cg-kbd>Esc</cg-kbd>'},{label:'Sizes (sm / md / lg)',html:'<cg-stack direction="row" gap="md" align="center"><cg-kbd size="sm" keys="⌘,K"></cg-kbd><cg-kbd size="md" keys="⌘,K"></cg-kbd><cg-kbd size="lg" keys="⌘,K"></cg-kbd></cg-stack>'},{label:'Inline in copy',html:'<cg-text size="sm">Press <cg-kbd keys="⌘,K" size="sm"></cg-kbd> to open the command palette, or <cg-kbd keys="⇧,⌘,P" size="sm"></cg-kbd> to jump to a file.</cg-text>'},{label:'Outline variant',html:'<cg-stack direction="row" gap="md"><cg-kbd keys="Ctrl,C" variant="outline"></cg-kbd><cg-kbd keys="Ctrl,V" variant="outline"></cg-kbd><cg-kbd keys="Ctrl,Z" variant="outline"></cg-kbd></cg-stack>'}], since:'v0.7.0' },
  { tag: 'cg-aspect-ratio', name: 'Aspect Ratio', category: 'layout', description: 'Layout primitive that constrains its slotted child (image, video, embed) to a fixed aspect ratio. Use the `fit` prop to control how the child fills the box (cover crops, contain letterboxes, etc.).', props: [{name:'ratio',type:'string',default:'"16/9"',description:'Aspect ratio expressed as W/H (e.g. "16/9", "1/1", "4/3")'},{name:'fit',type:'"cover" | "contain" | "fill" | "none" | "scale-down"',default:'"cover"',description:'How the slotted element fills the box (object-fit)'}], events: [], examples: [{label:'16:9 video frame',html:'<cg-aspect-ratio ratio="16/9" style="max-width:480px;"><img src="https://picsum.photos/480/270" alt="16:9" /></cg-aspect-ratio>'},{label:'Square avatar',html:'<cg-aspect-ratio ratio="1/1" style="max-width:200px;border-radius:9999px;overflow:hidden;"><img src="https://picsum.photos/200" alt="Avatar" /></cg-aspect-ratio>'},{label:'Portrait fit=contain (letterboxed logo)',html:'<cg-aspect-ratio ratio="3/4" fit="contain" style="max-width:240px;background:var(--bg-subtle);border:1px solid var(--border);border-radius:10px;"><img src="https://picsum.photos/240/240" alt="Logo" /></cg-aspect-ratio>'}], since:'v0.7.0' },
  { tag: 'cg-scroll-area', name: 'Scroll Area', category: 'layout', description: 'Custom-styled scroll container with themed scrollbars. Native scrolling with tokenized scrollbar thumbs — three visibility modes (auto / always / hover) and three orientations (vertical / horizontal / both).', props: [{name:'orientation',type:'"vertical" | "horizontal" | "both"',default:'"vertical"',description:'Scroll direction'},{name:'type',type:'"auto" | "always" | "hover"',default:'"hover"',description:'Scrollbar visibility — auto = on overflow, always = always visible, hover = only when hovering the area'}], events: [], examples: [{label:'Vertical (default · hover)',html:'<cg-scroll-area style="height:200px;width:360px;padding:16px;background:var(--cg-color-surface-cards-background);border:1px solid var(--cg-color-surface-cards-border);border-radius:10px;"><div style="display:flex;flex-direction:column;gap:8px;">' + Array.from({length:20}, (_,i) => `<div style="padding:10px;background:var(--cg-color-surface-container-background);border-radius:6px;font-size:13px;">Item ${i+1}</div>`).join('') + '</div></cg-scroll-area>'},{label:'Horizontal (always)',html:'<cg-scroll-area orientation="horizontal" type="always" style="height:96px;width:360px;padding:12px;background:var(--cg-color-surface-cards-background);border:1px solid var(--cg-color-surface-cards-border);border-radius:10px;"><div style="display:flex;gap:8px;">' + Array.from({length:14}, (_,i) => `<div style="flex-shrink:0;padding:18px 24px;background:var(--cg-color-surface-container-background);border-radius:6px;font-size:13px;white-space:nowrap;">Card ${i+1}</div>`).join('') + '</div></cg-scroll-area>'},{label:'Both (always — wide grid)',html:'<cg-scroll-area orientation="both" type="always" style="height:200px;width:360px;padding:12px;background:var(--cg-color-surface-cards-background);border:1px solid var(--cg-color-surface-cards-border);border-radius:10px;"><div style="display:grid;grid-template-columns:repeat(8,120px);gap:8px;">' + Array.from({length:48}, (_,i) => `<div style="padding:10px 14px;background:var(--cg-color-surface-container-background);border-radius:6px;font-size:13px;white-space:nowrap;">Item ${i+1}</div>`).join('') + '</div></cg-scroll-area>'},{label:'Hover-only scrollbar',html:'<cg-scroll-area type="hover" style="height:160px;width:360px;padding:16px;background:var(--cg-color-surface-cards-background);border:1px solid var(--cg-color-surface-cards-border);border-radius:10px;"><p style="margin:0 0 12px;font-size:13px;color:var(--cg-color-surface-container-outlined);">Hover the area to reveal the scrollbar.</p><div style="display:flex;flex-direction:column;gap:6px;">' + Array.from({length:20}, (_,i) => `<div style="padding:8px 10px;background:var(--cg-color-surface-container-background);border-radius:6px;font-size:13px;">Line ${i+1}</div>`).join('') + '</div></cg-scroll-area>'}], since:'v0.7.0' },
  { tag: 'cg-navbar', name: 'Navbar', category: 'navigation', description: 'Top navigation bar with animated sliding active indicator, programmatic items API, three coherent variants (solid / glass / floating), responsive mobile panel, scroll-aware compact mode, and full keyboard navigation (arrow keys / Home / End / Escape).', props: [{name:'items',type:'NavItem[]',default:'[]',description:'Primary nav items — drives the sliding indicator and tab semantics'},{name:'active',type:'string',default:'""',description:'value of the active NavItem (consumer wires to router)'},{name:'variant',type:'"solid" | "glass" | "floating"',default:'"solid"',description:'Visual treatment'},{name:'sticky',type:'boolean',default:'false',description:'Position-sticky to top'},{name:'bordered',type:'boolean',default:'false',description:'Bottom border'},{name:'compact-on-scroll',type:'boolean',default:'false',description:'Shrink height + add elevation after scroll past 16px'},{name:'mobile-open',type:'boolean',default:'false',description:'Mobile panel open state'},{name:'brand-href',type:'string',default:'""',description:'Makes the brand area a clickable anchor'}], events: [{name:'cg-navbar-toggle',detail:'{open: boolean}',description:'Mobile menu toggled'},{name:'cg-navbar-select',detail:'{value: string, item: NavItem}',description:'A nav item activated (click or Enter/Space)'}], examples: [{label:'Solid + sliding indicator',html:'<cg-navbar id="demo-solid" active="docs"><span slot="brand" style="display:flex;align-items:center;gap:8px;"><svg width="20" height="20" viewBox="0 0 16 16" fill="currentColor" style="color:var(--cg-color-accent-text);"><circle cx="8" cy="8" r="7"/></svg>Cognivo</span><cg-button slot="end" variant="tertiary" size="sm">Log in</cg-button><cg-button slot="end" variant="primary" size="sm">Sign up</cg-button></cg-navbar>',setup:(el)=>{const n=el.querySelector("cg-navbar");if(n)n.items=[{value:"docs",label:"Docs",href:"#"},{value:"components",label:"Components",href:"#"},{value:"tokens",label:"Tokens",href:"#",badge:"New"},{value:"pricing",label:"Pricing",href:"#"}];n?.addEventListener("cg-navbar-select",(e)=>{n.active=e.detail.value;});}},{label:'Glass + sticky',html:'<div style="height:280px;overflow-y:auto;background:linear-gradient(135deg,#1e1b4b,#312e81,#1e1b4b);border-radius:12px;"><cg-navbar id="demo-glass" variant="glass" sticky active="components"><span slot="brand" style="font-weight:700;">Glass</span><cg-button slot="end" variant="primary" size="sm">Try it</cg-button></cg-navbar><div style="padding:24px;color:#fff;font-size:13px;line-height:1.7;">Scroll inside this container to see the glass blur effect. The navbar stays sticky at the top with backdrop saturation and blur — every layer of content beneath shifts through the translucent surface.<br><br>' + Array.from({length:8},(_,i)=>`Block ${i+1} of background content.`).join("<br><br>") + '</div></div>',setup:(el)=>{const n=el.querySelector("cg-navbar");if(n)n.items=[{value:"home",label:"Home",href:"#"},{value:"components",label:"Components",href:"#"},{value:"docs",label:"Docs",href:"#"}];n?.addEventListener("cg-navbar-select",(e)=>{n.active=e.detail.value;});}},{label:'Floating pill',html:'<div style="background:radial-gradient(circle at 30% 50%,#312e81,#0f172a);padding:12px;border-radius:12px;"><cg-navbar id="demo-floating" variant="floating" active="home"><span slot="brand" style="display:flex;align-items:center;gap:8px;font-weight:600;"><svg width="18" height="18" viewBox="0 0 16 16" fill="currentColor" style="color:var(--cg-color-accent-text);"><circle cx="8" cy="8" r="7"/></svg>Floating</span><cg-button slot="end" variant="primary" size="sm">Try it</cg-button></cg-navbar></div>',setup:(el)=>{const n=el.querySelector("cg-navbar");if(n)n.items=[{value:"home",label:"Home",href:"#"},{value:"products",label:"Products",href:"#"},{value:"docs",label:"Docs",href:"#"}];n?.addEventListener("cg-navbar-select",(e)=>{n.active=e.detail.value;});}},{label:'With kbd hints + badge',html:'<cg-navbar id="demo-kbd" active="search" sticky bordered><span slot="brand" style="font-weight:700;letter-spacing:-0.02em;">⌘ Linear-style</span><cg-button slot="end" variant="primary" size="sm">New issue</cg-button></cg-navbar>',setup:(el)=>{const n=el.querySelector("cg-navbar");if(n)n.items=[{value:"inbox",label:"Inbox",href:"#",kbd:"⌘1"},{value:"search",label:"Search",href:"#",kbd:"⌘K"},{value:"projects",label:"Projects",href:"#",kbd:"⌘2",badge:"3"},{value:"team",label:"Team",href:"#",kbd:"⌘3"}];n?.addEventListener("cg-navbar-select",(e)=>{n.active=e.detail.value;});}},{label:'Disabled item + arrow-key nav',html:'<cg-navbar id="demo-keyboard" active="overview" bordered><span slot="brand" style="font-weight:700;">Try Arrow Keys</span></cg-navbar>',setup:(el)=>{const n=el.querySelector("cg-navbar");if(n)n.items=[{value:"overview",label:"Overview",href:"#"},{value:"reports",label:"Reports",href:"#"},{value:"admin",label:"Admin",href:"#",disabled:true},{value:"billing",label:"Billing",href:"#"}];n?.addEventListener("cg-navbar-select",(e)=>{n.active=e.detail.value;});}}], since:'v0.7.0' },
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    tag: 'cg-sidebar', name: 'Sidebar', category: 'navigation',
    description: 'Modern collapsible side navigation. Two states (expanded ↔ icon-only), smooth 200ms width transition, full-row hit targets on slotted `<a>` / `<button>` items, `aria-current` active state, and section headers via `.section-title`. Modeled after the 2024–2025 consensus from shadcn/ui, Linear, Notion, and Cursor.',
    props: [
      { name: 'collapsed', type: 'boolean', default: 'false', description: 'Collapsed to icon-only mode' },
      { name: 'collapsible', type: 'boolean', default: 'false', description: 'Show the collapse toggle button in the header' },
      { name: 'side', type: '"left" | "right"', default: '"left"', description: 'Which side the sidebar attaches to' },
      { name: 'sticky', type: 'boolean', default: 'false', description: 'Sticky positioning (top: 0)' },
      { name: 'width', type: 'string', default: '""', description: 'Custom width CSS value (overrides --cg-component-sidebar-width)' },
    ],
    events: [
      { name: 'cg-sidebar-toggle', detail: '{ collapsed: boolean }', description: 'Sidebar collapsed/expanded via toggle button' },
    ],
    examples: [
      {
        label: 'App shell with sections',
        html: `<cg-sidebar collapsible style="height:440px;max-width:280px;">
  ...
  <div slot="footer" style="display:flex;align-items:center;gap:10px;font-size:13px;">
    ...
    <span data-label>Murilo S.</span>
      ...
      {
        label: 'Right side (inspector)',
        html: `<cg-sidebar collapsible side="right" style="height:300px;max-width:260px;">
  ...
  <a href="#" title="Comments"><span data-label>Comments</span></a>
      ...
      {
        label: 'Static (no collapse)',
        html: `<cg-sidebar style="height:240px;max-width:240px;">
  ...
  { tag: 'cg-avatar', name: 'Avatar', category: 'data-display', description: 'User avatar with image, initials fallback, five sizes, circle/square shape, status indicator dot, and optional typed ring (user/agent/system) for AI-app shells.', props: [{name:'src',type:'string',default:'""',description:'Image source URL'},{name:'alt',type:'string',default:'""',description:'Image alt text'},{name:'name',type:'string',default:'""',description:'User name for initials fallback'},{name:'size',type:'"xs" | "sm" | "md" | "lg" | "xl"',default:'"md"',description:'Avatar size'},{name:'shape',type:'"circle" | "square"',default:'"circle"',description:'Avatar shape'},{name:'status',type:'"online" | "offline" | "away" | "busy" | null',default:'null',description:'Status indicator dot'},{name:'type',type:'"user" | "agent" | "system" | null',default:'null',description:'Adds a colored ring around the avatar (lime for `agent`, info for `user`, muted for `system`)'},{name:'fallback-icon',type:'string',default:'""',description:'Custom fallback icon'}], events: [], examples: [{label:'With image',html:'<cg-avatar src="https://i.pravatar.cc/80" name="Ada Lovelace" size="lg" status="online"></cg-avatar>'},{label:'Initials fallback',html:'<cg-avatar name="Grace Hopper" size="md"></cg-avatar>'},{label:'Square shape',html:'<cg-avatar name="Alan Turing" size="lg" shape="square" status="away"></cg-avatar>'},{label:'Typed rings (AI app)',html:'<cg-stack direction="row" gap="md"><cg-avatar name="Alice" type="user" status="online"></cg-avatar><cg-avatar name="Claude" type="agent" status="online"></cg-avatar><cg-avatar name="Sys" type="system"></cg-avatar></cg-stack>'}], since:'v0.9.0' },
  { tag: 'cg-status-dot', name: 'Status Dot', category: 'data-display', description: 'Small colored dot indicating status/presence, with an optional pulse animation and accessible label. Maps status to a tier-2 semantic color.', props: [{name:'status',type:'"online" | "away" | "busy" | "offline" | "success" | "warning" | "error" | "info" | "neutral"',default:'"neutral"',description:'Status → semantic color'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Dot diameter'},{name:'pulse',type:'boolean',default:'false',description:'Animated pulse ring (respects reduced motion)'},{name:'label',type:'string',default:'""',description:'Accessible label; when empty the dot is aria-hidden'}], events: [], examples: [{label:'Presence states',html:'<cg-stack direction="row" gap="md"><cg-status-dot status="online" label="Online"></cg-status-dot><cg-status-dot status="away" label="Away"></cg-status-dot><cg-status-dot status="busy" label="Busy"></cg-status-dot><cg-status-dot status="offline" label="Offline"></cg-status-dot></cg-stack>'},{label:'With pulse',html:'<cg-stack direction="row" gap="sm" align="center"><cg-status-dot status="online" pulse></cg-status-dot><cg-text>Live</cg-text></cg-stack>'},{label:'Sizes',html:'<cg-stack direction="row" gap="md" align="center"><cg-status-dot status="success" size="sm"></cg-status-dot><cg-status-dot status="success" size="md"></cg-status-dot><cg-status-dot status="success" size="lg"></cg-status-dot></cg-stack>'}], since:'v0.8.0' },
  { tag: 'cg-timestamp', name: 'Timestamp', category: 'data-display', description: 'Renders a point in time as relative ("2h ago") or absolute text inside a semantic <time> element, auto-refreshing relative values when live.', props: [{name:'datetime',type:'string',default:'""',description:'ISO 8601 string or epoch ms'},{name:'format',type:'"relative" | "datetime" | "date" | "time"',default:'"relative"',description:'Display mode'},{name:'live',type:'boolean',default:'false',description:'Auto-refresh relative value every 60s'}], events: [], examples: [{label:'Relative (live)',html:'<cg-timestamp datetime="2026-07-03T09:00:00Z" live></cg-timestamp>'},{label:'Absolute datetime',html:'<cg-timestamp datetime="2026-07-03T09:00:00Z" format="datetime"></cg-timestamp>'},{label:'Date only',html:'<cg-timestamp datetime="2026-07-03T09:00:00Z" format="date"></cg-timestamp>'}], since:'v0.8.0' },
  { tag: 'cg-thumbnail', name: 'Thumbnail', category: 'data-display', description: 'Small, fixed-size preview image with optional selectable/clickable affordances. For large content images use cg-image.', props: [{name:'src',type:'string',default:'""',description:'Image URL'},{name:'alt',type:'string',default:'""',description:'Alt text'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Fixed tile size'},{name:'rounded',type:'"none" | "sm" | "md" | "lg" | "full"',default:'"md"',description:'Corner radius'},{name:'selectable',type:'boolean',default:'false',description:'Render as a toggle (checkbox role)'},{name:'selected',type:'boolean',default:'false',description:'Selected state'},{name:'disabled',type:'boolean',default:'false',description:'Non-interactive, dimmed'}], events: [{name:'cg-thumbnail-select',detail:'{selected: boolean}',description:'Fired on toggle (selectable mode)'}], examples: [{label:'Basic',html:'<cg-stack direction="row" gap="sm"><cg-thumbnail src="https://i.pravatar.cc/80?img=1" alt="1"></cg-thumbnail><cg-thumbnail src="https://i.pravatar.cc/80?img=2" alt="2"></cg-thumbnail><cg-thumbnail alt="Empty"></cg-thumbnail></cg-stack>'},{label:'Selectable gallery',html:'<cg-stack direction="row" gap="sm"><cg-thumbnail src="https://i.pravatar.cc/80?img=3" alt="A" selectable selected></cg-thumbnail><cg-thumbnail src="https://i.pravatar.cc/80?img=4" alt="B" selectable></cg-thumbnail><cg-thumbnail src="https://i.pravatar.cc/80?img=5" alt="C" selectable></cg-thumbnail></cg-stack>'},{label:'Sizes & rounding',html:'<cg-stack direction="row" gap="sm" align="center"><cg-thumbnail src="https://i.pravatar.cc/80?img=6" alt="sm" size="sm"></cg-thumbnail><cg-thumbnail src="https://i.pravatar.cc/80?img=7" alt="lg" size="lg" rounded="full"></cg-thumbnail></cg-stack>'}], since:'v0.8.0' },
  { tag: 'cg-blockquote', name: 'Blockquote', category: 'typography', description: 'Editorial quotation primitive: a semantic <blockquote> anchored by an oversized decorative quote mark (pull-quote style), with an optional attribution footer. No left rule — the glyph carries the emphasis.', props: [{name:'cite',type:'string',default:'""',description:'Source URL (blockquote cite)'},{name:'variant',type:'"default" | "accent" | "muted"',default:'"default"',description:'default = neutral glyph; accent = brand-colored glyph + larger type; muted = soft filled card'}], events: [], examples: [{label:'Accent',html:'<cg-blockquote variant="accent">Design tokens are the contract between design and code.<span slot="footer">Cognivo docs</span></cg-blockquote>'},{label:'Muted card',html:'<cg-blockquote variant="muted">Think in systems, not screens.<span slot="footer">Design principles</span></cg-blockquote>'},{label:'Default',html:'<cg-blockquote>Every spacing value should come from the token scale.</cg-blockquote>'}], since:'v0.8.0' },
  { tag: 'cg-empty-state', name: 'Empty State', category: 'data-display', description: 'Empty/no-results/error state with contextual icon, title, description, and actions slot. Six variants — including `ai` for AI-flavored empty contexts (renders an accent bubble around the icon).', props: [{name:'variant',type:'"default" | "search" | "error" | "success" | "info" | "ai"',default:'"default"',description:'Visual variant — drives the contextual icon and color treatment'},{name:'title',type:'string',default:'""',description:'Title text'},{name:'description',type:'string',default:'""',description:'Description text'},{name:'icon',type:'string',default:'""',description:'Custom icon override'}], events: [], examples: [{label:'Search empty',html:'<cg-empty-state variant="search" title="No results found" description="Try a different search query or adjust your filters."><cg-button slot="actions" variant="secondary" size="sm">Clear filters</cg-button></cg-empty-state>'},{label:'Error state',html:'<cg-empty-state variant="error" title="Something went wrong" description="We could not load your data. Please try again."><cg-button slot="actions" variant="primary" size="sm">Retry</cg-button></cg-empty-state>'},{label:'AI variant',html:'<cg-empty-state variant="ai" title="No insights yet" description="Run an analysis to generate AI-powered insights from your data."><cg-button slot="actions" variant="primary" size="sm">Start Analysis</cg-button></cg-empty-state>'}], since:'v0.9.0' },
  ...
  { tag: 'ai-notification-center', name: 'Notifications', category: 'ai-production', description: 'Notification inbox with date-bucketed grouping (Today / Yesterday / This week / Earlier), left-accent unread style, pulse on new arrivals, positive empty state, and SVG dismiss icons.', props: [{name:'notifications',type:'{id, title, message, type, timestamp, read?}[]',description:'Notification list (timestamp = ms epoch or ISO string)'},{name:'max-visible',type:'number',default:'50',description:'Max items rendered at once'},{name:'rounded',type:'"none" | "sm" | "md" | "lg" | "full"',default:'"lg"',description:'Card border-radius variant'}], events: [{name:'ai-notification-click',detail:'{id, notification}',description:'Notification clicked'},{name:'ai-notification-dismiss',detail:'{id}',description:'× clicked on a notification'},{name:'ai-notification-read-all',detail:'{}',description:'"Mark all read" clicked'}], examples: [{label:'Inbox with all date buckets',html:'<ai-notification-center style="max-width:420px;"></ai-notification-center>',setup:(el)=>{const n=el.querySelector('ai-notification-center') as any;if(n){const now=Date.now();n.notifications=[{id:'1',title:'Analysis Complete',message:'Q4 revenue report is ready to review.',type:'success',timestamp:now-60000},{id:'2',title:'Rate Limit Warning',message:'80% of daily quota used. Consider upgrading.',type:'warning',timestamp:now-1800000},{id:'3',title:'Model Update',message:'Claude 3.5 Sonnet v2 available.',type:'info',timestamp:now-18000000,read:true},{id:'4',title:'Webhook delivery failed',message:'Retry exhausted after 5 attempts.',type:'error',timestamp:now-93600000},{id:'5',title:'Weekly digest',message:'Your team shipped 47 AI completions last week.',type:'info',timestamp:now-345600000,read:true}];}}},{label:'Empty inbox',html:'<ai-notification-center style="max-width:420px;"></ai-notification-center>'}], since:'v0.5.0' },
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  {
    tag: 'ai-presence', name: 'Presence', category: 'ai-collab',
    description: 'Overlapping avatar stack showing live user presence with status dots (online/away/busy/offline), pulsing online indicator, hover tooltips with status + last-seen text, and a +N overflow badge. Three sizes (sm/md/lg) and an async-loading skeleton state.',
    props: [
      { name: 'users', type: '{ name, status, avatar?, lastSeen? }[]', description: 'User presence list' },
      { name: 'size', type: '"sm" | "md" | "lg"', default: '"md"', description: 'Avatar size — scales dot size, overlap, and initials font' },
      { name: 'max-visible', type: 'number', default: '5', description: 'Max avatars shown before overflow badge' },
      { name: 'loading', type: 'boolean', default: 'false', description: 'Render skeleton placeholders instead of users' },
    ],
    events: [
      { name: 'ai-presence-user-click', detail: '{ user }', description: 'An avatar was clicked' },
    ],
    examples: [
      {
        label: 'All four statuses',
        html: '<ai-presence></ai-presence>',
        setup: (el) => {
          const p = el.querySelector('ai-presence') as any;
          if (p) p.users = [
      ...
      },
      {
        ...
        html: '<cg-stack direction="column" gap="md"><ai-presence size="sm"></ai-presence><ai-presence size="md"></ai-presence><ai-presence size="lg"></ai-presence></cg-stack>',
        setup: (el) => {
          const users = [
            { name: 'Alice', status: 'online' },
            { name: 'Bob', status: 'away' },
            { name: 'Carol', status: 'busy' },
            { name: 'Dave', status: 'online' },
          ];
          el.querySelectorAll('ai-presence').forEach((p: any) => { p.users = users; });
        },
      },
      {
        label: 'With overflow (+N)',
        html: '<ai-presence max-visible="3"></ai-presence>',
        setup: (el) => {
          const p = el.querySelector('ai-presence') as any;
          if (p) p.users = [
            { name: 'Alice', status: 'online' },
      ...
      },
      {
        ...
        html: '<ai-presence loading max-visible="4"></ai-presence>',
      },
    ],
    since: 'v0.5.0',
  },
  ...
  { tag: 'ai-onboarding', name: 'Onboarding', category: 'ai-collab', description: 'Step-by-step tutorial with clickable progress dots, media + per-step CTA slots, crossfade transitions, arrow-key + Esc navigation, and aria-live announcements.', props: [{name:'steps',type:'{title, description}[]',description:'Tutorial steps'},{name:'active',type:'number',default:'0',description:'Current step index'},{name:'dismissible',type:'boolean',default:'true',description:'Show close button + enable Esc'},{name:'show-skip',type:'boolean',default:'true',description:'Show Skip tour link (hidden on last step)'},{name:'rounded',type:'"none" | "sm" | "md" | "lg" | "full"',default:'"lg"',description:'Card border-radius variant'},{name:'progress',type:'"auto" | "numbered" | "dots" | "bar" | "stepper"',default:'"auto"',description:'Progress affordance — auto picks dots for ≤4 steps, bar for 5+'}], events: [{name:'ai-onboarding-next',detail:'{step}',description:'Next clicked'},{name:'ai-onboarding-prev',detail:'{step}',description:'Back clicked'},{name:'ai-onboarding-step-change',detail:'{from, to}',description:'Any step change (incl. dot click)'},{name:'ai-onboarding-complete',detail:'{}',description:'Done clicked on last step'},{name:'ai-onboarding-dismiss',detail:'{step}',description:'Dismissed via close, Skip, or Esc'}], examples: [{label:'Dots (default for ≤4 steps)',html:'<ai-onboarding progress="dots" active="1" dismissible style="max-width:420px;"></ai-onboarding>',setup:(el)=>{const o=el.querySelector('ai-onboarding') as any;if(o)o.steps=[{title:'Welcome to Cognivo',description:'Build AI-native interfaces with cognitive design intelligence.'},{title:'Ask in natural language',description:'Describe a UI in plain English; the streaming renderer assembles it.'},{title:'Inspect bias signals',description:'Every component is annotated with the cognitive biases it engages.'},{title:'Ship with confidence',description:'Token-governed CSS, WCAG AA contrast, SSR-ready.'}];}},{label:'Numbered (eyebrow only)',html:'<ai-onboarding progress="numbered" active="1" dismissible style="max-width:420px;"></ai-onboarding>',setup:(el)=>{const o=el.querySelector('ai-onboarding') as any;if(o)o.steps=[{title:'Connect your data',description:'Plug in a database, warehouse, or API endpoint.'},{title:'Configure your model',description:'Pick a base model and tune the system prompt.'},{title:'Test & deploy',description:'Validate outputs, then ship to production.'}];}},{label:'Bar (default for 5+ steps)',html:'<ai-onboarding progress="bar" active="2" dismissible style="max-width:420px;"></ai-onboarding>',setup:(el)=>{const o=el.querySelector('ai-onboarding') as any;if(o)o.steps=[{title:'Welcome',description:'Quick tour of the platform.'},{title:'Connect data',description:'Wire up a source.'},{title:'Configure agent',description:'Pick model and tools.'},{title:'Set guardrails',description:'Define the agent permissions and forbidden actions.'},{title:'Test runs',description:'Validate behavior against real prompts.'},{title:'Deploy & monitor',description:'Ship and watch the dashboards.'}];}},{label:'Stepper (numbered nodes)',html:'<ai-onboarding progress="stepper" active="2" dismissible style="max-width:440px;"></ai-onboarding>',setup:(el)=>{const o=el.querySelector('ai-onboarding') as any;if(o)o.steps=[{title:'Account',description:'Create your workspace.'},{title:'Team',description:'Invite collaborators.'},{title:'Integrations',description:'Connect Slack, GitHub, Linear.'},{title:'Launch',description:'Ready to roll.'}];}}], since:'v0.5.0' },
  {
    tag: 'ai-usage-meter', name: 'Usage Meter', category: 'ai-collab',
    description: 'Quota / rate-limit ring meter with step thresholds (75% warning, 90% danger), animated count-up, sub-24h relative countdown, threshold-crossing aria-live announcements, pulsing glow at danger tier, and three sizes (compact 24px chip → default 96px → large 128px hero). Built from cg-card + cg-text + cg-button primitives.',
    props: [
      { name: 'used', type: 'number', default: '0', description: 'Current usage count' },
      { name: 'limit', type: 'number', default: '100', description: 'Maximum quota' },
      { name: 'label', type: 'string', default: '"Usage"', description: 'Metric label' },
      { name: 'unit', type: 'string', default: '"requests"', description: 'Unit text' },
      { name: 'reset-date', type: 'string', default: '""', description: 'ISO date or human string for quota reset; sub-24h auto-formats as a live "Resets in 3h 15m" countdown' },
      { name: 'size', type: '"default" | "compact" | "lg"', default: '"default"', description: 'compact = 24px chip for nav-bars; lg = 128px hero ring for billing pages' },
    ],
    events: [
      { name: 'ai-usage-upgrade', detail: '{}', description: 'Upgrade CTA clicked (visible when usage ≥ 80%)' },
      { name: 'ai-usage-threshold', detail: '{ tier: "normal" | "warning" | "danger" }', description: 'Crossed the 75% or 90% threshold' },
    ],
    examples: [
      {
        label: 'Healthy (under 75%)',
        html: `<ai-usage-meter used="4200" limit="10000" label="API Requests" unit="requests"></ai-usage-meter>`,
      },
      {
        label: 'Warning (≥75%) with live countdown',
        html: `<ai-usage-meter used="8200" limit="10000" label="API Requests" unit="requests" reset-date="${new Date(Date.now() + 3 * 60 * 60 * 1000 + 12 * 60 * 1000).toISOString()}"></ai-usage-meter>`,
      },
      {
        label: 'Danger (≥90%) — pulsing ring',
        html: `<ai-usage-meter used="9650" limit="10000" label="API Requests" unit="requests" reset-date="${new Date(Date.now() + 26 * 60 * 60 * 1000).toISOString()}"></ai-usage-meter>`,
      },
      {
        label: 'Large hero (billing page)',
        html: `<ai-usage-meter size="lg" used="14200" limit="20000" label="Monthly tokens" unit="tokens" reset-date="${new Date(Date.now() + 12 * 24 * 60 * 60 * 1000).toISOString()}"></ai-usage-meter>`,
      },
      {
        label: 'Compact chip (nav-bar)',
        html: `<cg-stack direction="row" gap="md" align="center"><ai-usage-meter size="compact" used="6700" limit="10000"></ai-usage-meter><cg-text size="sm" color="muted">tokens used</cg-text></cg-stack>`,
      },
    ],
    since: 'v0.5.0',
  },
  { tag: 'ai-model-comparison', name: 'Model Comparison', category: 'ai-collab', description: 'Side-by-side AI model comparison table with per-metric "Best" pill, sticky first column, gradient score bars, and staggered fade-in.', props: [{name:'models',type:'{name, provider, scores, costTier, contextWindow}[]',description:'Models to compare'},{name:'rounded',type:'"none" | "sm" | "md" | "lg" | "full"',default:'"lg"',description:'Wrapper border-radius variant'}], events: [{name:'ai-comparison-select',detail:'{model}',description:'Fired when a model row Select button is clicked'}], examples: [{label:'Compare three',html:'<ai-model-comparison></ai-model-comparison>',setup:(el)=>{const m=el.querySelector('ai-model-comparison') as any;if(m)m.models=[{name:'GPT-4o',provider:'OpenAI',scores:{reasoning:92,coding:88,speed:75,multilingual:86},costTier:'$$$',contextWindow:128000},{name:'Claude 3.5',provider:'Anthropic',scores:{reasoning:95,coding:92,speed:82,multilingual:89},costTier:'$$',contextWindow:200000},{name:'Gemini 1.5',provider:'Google',scores:{reasoning:88,coding:84,speed:90,multilingual:91},costTier:'$',contextWindow:1000000}];}}], since:'v0.5.0' },
  ...
  { tag: 'ai-status-page', name: 'Status Page', category: 'ai-collab', description: 'Service health dashboard with overall status banner, per-service rows, optional 90-day uptime calendar bars, latency bars, pulsing dots on degraded/down, last-updated timestamp, and active-incident slot.', props: [{name:'services',type:'{name, status, latency?, uptime?, history?}[]',description:'Service entries'},{name:'last-updated',type:'string',default:'""',description:'ISO date / number / Date — rendered as relative time (e.g., "Updated 12s ago")'}], events: [{name:'ai-status-service-click',detail:'{service}',description:'Service row clicked'}], examples: [{label:'Healthy — no incidents',html:'<ai-status-page style="max-width:560px;"></ai-status-page>',setup:(el)=>{const s=el.querySelector('ai-status-page') as any;if(s){const today=Date.now();const mk=(fail=[])=>Array.from({length:90},(_,i)=>{const di=89-i;return{date:new Date(today-di*86400000).toISOString().slice(0,10),status:fail.includes(di)?(di%2?'degraded':'down'):'operational',incidents:fail.includes(di)?1:0};});s.services=[{name:'AI Chat API',status:'operational',latency:120,uptime:99.98,history:mk()},{name:'Embedding Service',status:'operational',latency:45,uptime:99.99,history:mk()},{name:'Vector Database',status:'operational',latency:8,uptime:99.95,history:mk()}];s.lastUpdated=new Date(today-12000);}}},{label:'Partial degradation',html:'<ai-status-page style="max-width:560px;"></ai-status-page>',setup:(el)=>{const s=el.querySelector('ai-status-page') as any;if(s){const today=Date.now();const mk=(fail=[])=>Array.from({length:90},(_,i)=>{const di=89-i;return{date:new Date(today-di*86400000).toISOString().slice(0,10),status:fail.includes(di)?(di%2?'degraded':'down'):'operational',incidents:fail.includes(di)?1:0};});s.services=[{name:'AI Chat API',status:'operational',latency:120,uptime:99.98,history:mk([45])},{name:'Image Generation',status:'degraded',latency:2400,uptime:98.5,history:mk([0,2,12,28])},{name:'Webhooks',status:'maintenance',latency:95,uptime:99.7,history:mk()}];s.lastUpdated=new Date(today-5*60000);}}}], since:'v0.5.0' },
  { tag: 'ai-empty-state', name: 'Empty State', category: 'ai-collab', description: 'Contextual empty state with icon, title, description, and optional action button. Four semantic variants for different empty contexts (default / error / search / ai).', props: [{name:'icon',type:'string',default:'""',description:'Icon SVG markup (slot the default if empty)'},{name:'title',type:'string',default:'"Nothing here yet"',description:'Heading'},{name:'description',type:'string',default:'""',description:'Supporting body text'},{name:'action-label',type:'string',default:'""',description:'Action button text (omit to hide)'},{name:'variant',type:'"default" | "error" | "search" | "ai"',default:'"default"',description:'Visual variant — drives icon-circle bg, title color, and action-button color'}], events: [{name:'ai-empty-action',detail:'{}',description:'Action button clicked'}], examples: [{label:'Default — no data yet',html:'<ai-empty-state title="No data yet" description="Connect a data source to start analyzing." action-label="Connect Data"></ai-empty-state>'},{label:'AI variant',html:'<ai-empty-state variant="ai" title="No insights yet" description="Run an analysis to generate AI-powered insights from your data." action-label="Start Analysis"></ai-empty-state>'},{label:'Search — no results',html:'<ai-empty-state variant="search" title="No matches found" description="Try a different search query or adjust your filters." action-label="Clear filters"></ai-empty-state>'},{label:'Error state',html:'<ai-empty-state variant="error" title="Something went wrong" description="We couldn\\\'t load your data. Please try again." action-label="Retry"></ai-empty-state>'},{label:'No action — informational only',html:'<ai-empty-state variant="ai" title="Waiting for input" description="Start typing or upload a file to begin."></ai-empty-state>'}], since:'v0.5.0' },
  ...
  { tag: 'ai-test-runner', name: 'Test Runner', category: 'ai-devops', description: 'AI evaluation test results panel built from cg-card / cg-text / cg-button. Pass/fail/running/pending status icons, summary bar with combined progress, expandable expected/actual details, and a "Run All" CTA.', props: [{name:'tests',type:'{name, status, duration?, score?, expected?, actual?}[]',description:'Test entries'},{name:'title',type:'string',default:'"Test Results"',description:'Panel header title'}], events: [{name:'ai-test-run',detail:'{tests}',description:'"Run All" clicked — payload is the array of test names'},{name:'ai-test-click',detail:'{index, test}',description:'Test row toggled (expand/collapse)'}], examples: [{label:'Eval results with failures',html:'<ai-test-runner title="Model Evaluation" style="max-width:520px;"></ai-test-runner>',setup:(el)=>{const t=el.querySelector('ai-test-runner') as any;if(t)t.tests=[{name:'Relevance check',status:'pass',duration:120,score:92},{name:'Coherence',status:'pass',duration:85,score:88},{name:'Hallucination check',status:'fail',duration:340,score:45,expected:'No fabricated facts',actual:'Found 2 unverifiable claims'},{name:'Safety filter',status:'pass',duration:50,score:98},{name:'Format compliance',status:'running'},{name:'Multilingual robustness',status:'pending'}];}}], since:'v0.5.0' },
  ...
  {
    tag: 'ai-accessibility-report', name: 'A11y Report', category: 'ai-devops',
    description: 'WCAG compliance report with score ring, severity breakdown, expandable issues, and optional "How to fix" suggestions per issue. Built on cg-card. Modeled after Lighthouse / axe DevTools / Accessibility Insights.',
    props: [
      { name: 'issues', type: '{ rule, level, severity, description, element?, fix? }[]', description: 'List of WCAG issues found' },
      { name: 'score', type: 'number', default: '0', description: 'Overall accessibility score 0–100 (drives ring color: red < 50, amber < 90, green ≥ 90)' },
      { name: 'title', type: 'string', default: '"Accessibility Report"', description: 'Report title' },
      { name: 'total-checks', type: 'number', default: '0', description: 'Total checks run; renders "X of Y checks passed" header. Defaults to issues.length when 0' },
    ],
    events: [
      { name: 'ai-a11y-issue-click', detail: '{ issue, index }', description: 'An issue row was clicked/expanded' },
    ],
    examples: [
      {
        label: 'Page audit with fixes',
        html: '<ai-accessibility-report title="Page Audit" score="78" total-checks="24" style="max-width:560px;"></ai-accessibility-report>',
        setup: (el) => {
          const r = el.querySelector('ai-accessibility-report') as any;
          if (r) r.issues = [
            { rule: 'color-contrast', level: 'AA', severity: 'error', element: '<button class="cta">', description: 'Text has insufficient contrast ratio (3.2:1, minimum is 4.5:1).', fix: 'Increase the text color contrast against its background to at least 4.5:1.' },
            { rule: 'image-alt', level: 'A', severity: 'error', element: '<img src="/hero.png">', description: '3 images are missing alt attributes.', fix: 'Add a descriptive `alt` attribute to each image, or `alt=""` for decorative images.' },
            { rule: 'button-name', level: 'A', severity: 'warning', element: '<button>', description: 'Button has no accessible name (no text content, aria-label, or aria-labelledby).', fix: 'Add visible text inside the button or set `aria-label="..."`.' },
            { rule: 'heading-order', level: 'AA', severity: 'info', description: 'Heading levels skip from h2 to h4. Consider using h3 instead.' },
          ];
        },
      },
      {
        label: 'Perfect score (no issues)',
        html: '<ai-accessibility-report title="Production Build" score="100" total-checks="42" style="max-width:560px;"></ai-accessibility-report>',
      },
      {
        label: 'Critical (multiple errors)',
        html: '<ai-accessibility-report title="Landing Page" score="42" total-checks="18" style="max-width:560px;"></ai-accessibility-report>',
        setup: (el) => {
          const r = el.querySelector('ai-accessibility-report') as any;
          if (r) r.issues = [
            { rule: 'color-contrast', level: 'AA', severity: 'error', description: 'Multiple text elements fail contrast.', fix: 'Use #1a1a1a on white instead of #555.' },
            { rule: 'aria-required-attr', level: 'A', severity: 'error', description: 'Required ARIA attributes missing on 4 elements.' },
            { rule: 'image-alt', level: 'A', severity: 'error', description: '7 images without alt text.' },
            { rule: 'label', level: 'A', severity: 'error', description: 'Form inputs missing labels.' },
            { rule: 'duplicate-id', level: 'A', severity: 'warning', description: '2 duplicate id values in the DOM.' },
          ];
        },
      },
    ],
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  { tag: 'ai-app-sidebar', name: 'AI App Sidebar', category: 'ai-essentials', description: 'Collapsible AI app shell sidebar with data-driven sections, item icons/badges, and icon-only collapsed mode. Renamed from `ai-sidebar` to clarify its role vs. the slot-based `cg-sidebar` foundation.', props: [{name:'sections',type:'{title, items}[]',description:'Sidebar sections'},{name:'collapsed',type:'boolean',default:'false',description:'Collapsed mode'},{name:'activeId',type:'string',description:'Active item ID'}], events: [{name:'ai-app-sidebar-item-click',detail:'{id, label}',description:'Item clicked'},{name:'ai-app-sidebar-collapse',detail:'{collapsed}',description:'Collapse toggled'}], examples: [{label:'App sidebar',html:'<ai-app-sidebar style="max-width:250px;height:300px;"></ai-app-sidebar>',setup:(el)=>{const s=el.querySelector('ai-app-sidebar') as any;if(s){s.activeId='dashboard';s.sections=[{title:'Main',items:[{id:'dashboard',label:'Dashboard',icon:'DAS'},{id:'chat',label:'AI Chat',icon:'MSG'},{id:'data',label:'Data',icon:'DIR'}]},{title:'Settings',items:[{id:'team',label:'Team',icon:'USR'},{id:'billing',label:'Billing',icon:'PAY',badge:'3'}]}];}}}], since:'v0.5.0' },
  { tag: 'ai-sidebar', name: 'Sidebar (deprecated)', category: 'ai-essentials', description: 'Deprecated alias of `ai-app-sidebar` — wraps the renamed element and re-fires events under the legacy names. Use `ai-app-sidebar` directly; this shim will be removed in v0.6.0.', props: [{name:'sections',type:'{title, items}[]',description:'Sidebar sections'},{name:'collapsed',type:'boolean',default:'false',description:'Collapsed mode'},{name:'activeId',type:'string',description:'Active item ID'}], events: [{name:'ai-sidebar-item-click',detail:'{id, label}',description:'Item clicked (legacy event, preserved)'},{name:'ai-sidebar-collapse',detail:'{collapsed}',description:'Collapse toggled (legacy event, preserved)'}], examples: [{label:'Legacy sidebar',html:'<ai-sidebar style="max-width:250px;height:300px;"></ai-sidebar>',setup:(el)=>{const s=el.querySelector('ai-sidebar') as any;if(s){s.activeId='dashboard';s.sections=[{title:'Main',items:[{id:'dashboard',label:'Dashboard',icon:'DAS'},{id:'chat',label:'AI Chat',icon:'MSG'}]}];}}}], since:'v0.5.0' },
  { tag: 'ai-badge', name: 'AI Badge (deprecated)', category: 'ai-display', description: 'Deprecated alias of `ai-confidence-badge` — wraps the renamed element and re-fires events under the legacy name. Use `ai-confidence-badge` directly; this shim will be removed in v0.6.0.', props: [{name:'score',type:'number',default:'0.85',description:'Confidence 0-1'},{name:'showPercentage',type:'boolean',default:'true',description:'Show percentage label'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Size variant'},{name:'explanation',type:'string',description:'Tooltip explanation'},{name:'history',type:'number[]',description:'History for sparkline'},{name:'highThreshold',type:'number',default:'0.8',description:'High threshold'},{name:'lowThreshold',type:'number',default:'0.5',description:'Low threshold'}], events: [{name:'ai-badge-click',detail:'{score, level}',description:'Badge clicked (legacy event, preserved)'}], examples: [{label:'Legacy badge',html:'<ai-badge score="0.92" size="sm"></ai-badge>'}], since:'v0.1.0' },
  { tag: 'ai-action-receipt', name: 'Action Receipt', category: 'ai-workflow', description: 'Receipt for an agent action that already executed: what ran, what it touched, and the specific compensating affordance for this action (not a global undo). Status is labelled with text, never color alone (WCAG 1.4.1).', props: [{name:'summary',type:'string',description:'What the action did'},{name:'touched',type:'string[]',description:'Resources the action touched'},{name:'status',type:'"executed" | "failed"',default:'"executed"',description:'Action outcome'},{name:'error',type:'string',description:'Error message when status is failed'},{name:'compensationLabel',type:'string',description:'When set, a compensation button with this label is shown'}], events: [{name:'ai-action-compensate',detail:'void',description:'User asked to run the compensating action'}], examples: [{label:'Executed action',html:'<ai-action-receipt summary="Sent invoice #1042 to Acme Corp" compensationLabel="Void invoice" style="max-width:400px;"></ai-action-receipt>'}], since:'v0.6.0' },
  ...
  { tag: 'ai-permission-gate', name: 'Permission Gate', category: 'ai-production', description: 'Role-based access control card built from cg-card / cg-text / cg-button / cg-badge primitives. Allow/deny icon per feature, "Request Access" CTA on denied items, and an allowed/denied summary footer.', props: [{name:'permissions',type:'{feature, role, allowed, reason?}[]',description:'Permission entries'},{name:'current-role',type:'string',default:'""',description:'Active role to filter by (empty = show all)'},{name:'rounded',type:'"none" | "sm" | "md" | "lg" | "full"',default:'"lg"',description:'Card border-radius variant'}], events: [{name:'ai-permission-request',detail:'{feature, role}',description:'Request Access clicked on a denied feature'}], examples: [{label:'Editor role',html:'<ai-permission-gate current-role="editor" style="max-width:440px;"></ai-permission-gate>',setup:(el)=>{const p=el.querySelector('ai-permission-gate') as any;if(p)p.permissions=[{feature:'AI Chat',role:'editor',allowed:true},{feature:'Model Selection',role:'editor',allowed:true},{feature:'Fine-tuning',role:'editor',allowed:false,reason:'Admin only'},{feature:'API Keys',role:'editor',allowed:false,reason:'Owner only'},{feature:'Billing',role:'editor',allowed:false,reason:'Owner only'}];}}], since:'v0.5.0' },
  ...
  {
    tag: 'ai-assistant-widget', name: 'Assistant Widget', category: 'ai-collab',
    description: 'Floating embedded chat widget with FAB trigger, expandable panel, message list, and input area. Uses the modal-container surface family for visual consistency with cg-modal/cg-popover.',
    props: [
      { name: 'expanded', type: 'boolean', default: 'false', description: 'Expanded panel state' },
      { name: 'position', type: '"bottom-right" | "bottom-left"', default: '"bottom-right"', description: 'Widget anchor' },
      { name: 'welcomeMessage', type: 'string', default: '"How can I help you?"', description: 'Greeting shown when message list is empty' },
      { name: 'title', type: 'string', default: '"Assistant"', description: 'Panel header title' },
      { name: 'messages', type: '{ role: "ai" | "user", content: string }[]', description: 'Message transcript' },
    ],
    events: [
      { name: 'ai-assistant-open', detail: '{}', description: 'Panel expanded' },
      { name: 'ai-assistant-close', detail: '{}', description: 'Panel collapsed' },
      { name: 'ai-assistant-send', detail: '{ message: string }', description: 'User submitted a message' },
    ],
    examples: [
      {
        label: 'Expanded with conversation',
        html: '<div style="position:relative;height:560px;width:100%;"><ai-assistant-widget expanded title="AI Help" style="position:absolute;"></ai-assistant-widget></div>',
        setup: (el) => {
          const w = el.querySelector('ai-assistant-widget') as any;
          if (w) w.messages = [
            { role: 'ai', content: 'Hello! How can I assist you today?' },
            { role: 'user', content: 'How do I create a new component?' },
            { role: 'ai', content: 'Use the scaffold CLI: `cg generate component my-component`. It sets up Shadow DOM, tokens, and a test file.' },
          ];
        },
      },
      {
        label: 'Empty (welcome message)',
        html: '<div style="position:relative;height:560px;width:100%;"><ai-assistant-widget expanded title="Support" welcomeMessage="Ask me anything about Cognivo!" style="position:absolute;"></ai-assistant-widget></div>',
      },
      {
        label: 'Collapsed FAB',
        html: '<div style="position:relative;height:120px;width:100%;"><ai-assistant-widget style="position:absolute;" title="Support"></ai-assistant-widget></div>',
      },
    ],
    since: 'v0.6.0',
  },
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  { tag: 'cg-combobox', name: 'Combobox', category: 'forms', description: 'Autocomplete combobox with search, multi-select, loading state, and keyboard navigation. Shares input-field styling with cg-input (size, rounded, error, success states).', props: [{name:'options',type:'ComboOption[]',description:'Option list'},{name:'value',type:'string | string[]',description:'Selected value(s)'},{name:'multiple',type:'boolean',default:'false',description:'Multi-select mode'},{name:'placeholder',type:'string',description:'Placeholder text'},{name:'searchable',type:'boolean',default:'true',description:'Enable search filter'},{name:'clearable',type:'boolean',default:'false',description:'Show clear button'},{name:'disabled',type:'boolean',default:'false',description:'Disabled state'},{name:'loading',type:'boolean',default:'false',description:'Loading state'},{name:'error',type:'boolean',default:'false',description:'Error state — red border + error focus glow'},{name:'success',type:'boolean',default:'false',description:'Success state — success border + success focus glow'},{name:'size',type:'"md" | "lg"',default:'"md"',description:'Field height — matches cg-input sizes'},{name:'rounded',type:'"none" | "sm" | "md" | "lg"',default:'"lg"',description:'Border radius — matches cg-input rounded variants'}], events: [{name:'cg-combobox-change',detail:'{value}',description:'Selection changed'}], examples: [{label:'Basic',html:'<cg-combobox placeholder="Select a framework"></cg-combobox>',setup:(el)=>{const c=el.querySelector('cg-combobox') as any;if(c)c.options=[{label:'React',value:'react'},{label:'Vue',value:'vue'},{label:'Svelte',value:'svelte'}];}},{label:'Multi-select with chips',html:'<cg-combobox multiple clearable placeholder="Pick frameworks"></cg-combobox>',setup:(el)=>{const c=el.querySelector('cg-combobox') as any;if(c){c.options=[{label:'React',value:'react'},{label:'Vue',value:'vue'},{label:'Svelte',value:'svelte'},{label:'Solid',value:'solid'}];c.value=['react','vue'];}}},{label:'Error state',html:'<cg-combobox error placeholder="Required"></cg-combobox>',setup:(el)=>{const c=el.querySelector('cg-combobox') as any;if(c)c.options=[{label:'A',value:'a'},{label:'B',value:'b'}];}},{label:'Large size',html:'<cg-combobox size="lg" placeholder="Larger field"></cg-combobox>',setup:(el)=>{const c=el.querySelector('cg-combobox') as any;if(c)c.options=[{label:'One',value:'1'},{label:'Two',value:'2'}];}}], since:'v0.8.0' },
  { tag: 'cg-menubar', name: 'Menubar', category: 'navigation', description: 'App-style horizontal menubar with dropdown submenus. Implements the W3C WAI-ARIA Authoring Practices menubar pattern: roving tabindex (Tab exits the bar), Arrow keys navigate top/sub levels, Right/Left cross between submenus, Enter/Space/ArrowDown opens, Escape closes, Home/End jump to first/last, and typeahead jumps to items by first character. Submenu visuals shared with cg-dropdown / cg-split-button via the menu-list styling primitive.', props: [{name:'items',type:'MenubarItem[]',description:'Top-level menus, each with `label`, optional `id`, and a `children` array of `MenubarChildItem` (label, shortcut?, id?, disabled?, danger?, separator?)'},{name:'rounded',type:'"none" | "sm" | "md" | "lg" | "full"',default:'"md"',description:'Border radius — straight (none), small/medium/large, or fully rounded (pill). When "full", triggers also become pills so corners don\'t clash.'},{name:'size',type:'"sm" | "md" | "lg"',default:'"md"',description:'Compact / default / roomy sizing — drives bar padding, gap, and trigger font/padding.'},{name:'indicator',type:'"underline" | "none"',default:'"underline"',description:'Active-menu indicator style. "underline" — magnetic 2px bar slides between open menus (modern web feel, à la Notion / Vercel / Linear). "none" — rely only on the open trigger background (canonical macOS / VSCode / Figma app-menubar look).'}], events: [{name:'cg-menubar-select',detail:'{ menu: string, item: string }',description:'A submenu item was selected. Both fields fall back to the label when no id is set.'}], examples: [{label:'App menubar (File / Edit / View / Help)',html:'<cg-menubar style="max-width:480px;"></cg-menubar>',setup:(el)=>{const m=el.querySelector('cg-menubar') as any;if(m)m.items=[{label:'File',children:[{id:'new',label:'New file',shortcut:'⌘N'},{id:'open',label:'Open…',shortcut:'⌘O'},{id:'save',label:'Save',shortcut:'⌘S'},{id:'save-as',label:'Save as…',shortcut:'⌘⇧S'},{separator:true,label:''},{id:'close',label:'Close window',shortcut:'⌘W'},{id:'quit',label:'Quit',shortcut:'⌘Q',danger:true}]},{label:'Edit',children:[{id:'undo',label:'Undo',shortcut:'⌘Z'},{id:'redo',label:'Redo',shortcut:'⌘⇧Z'},{separator:true,label:''},{id:'cut',label:'Cut',shortcut:'⌘X'},{id:'copy',label:'Copy',shortcut:'⌘C'},{id:'paste',label:'Paste',shortcut:'⌘V'},{id:'paste-special',label:'Paste special…',disabled:true}]},{label:'View',children:[{id:'zoom-in',label:'Zoom in',shortcut:'⌘+'},{id:'zoom-out',label:'Zoom out',shortcut:'⌘−'},{id:'reset-zoom',label:'Reset zoom',shortcut:'⌘0'},{separator:true,label:''},{id:'fullscreen',label:'Toggle fullscreen',shortcut:'F11'}]},{label:'Help',children:[{id:'docs',label:'Documentation'},{id:'shortcuts',label:'Keyboard shortcuts',shortcut:'⌘/'},{separator:true,label:''},{id:'about',label:'About Cognivo'}]}];}},{label:'Try the keyboard',html:`
<cg-stack direction="column" gap="md">
  <cg-card padding="md">
...
`.trim(),setup:(el)=>{const m=el.querySelector('cg-menubar') as any;if(m)m.items=[{label:'File',children:[{id:'new',label:'New',shortcut:'⌘N'},{id:'open',label:'Open',shortcut:'⌘O'},{id:'save',label:'Save',shortcut:'⌘S'}]},{label:'Edit',children:[{id:'undo',label:'Undo',shortcut:'⌘Z'},{id:'redo',label:'Redo',shortcut:'⌘⇧Z'}]},{label:'View',children:[{id:'zoom-in',label:'Zoom in'},{id:'zoom-out',label:'Zoom out'}]},{label:'Help',children:[{id:'about',label:'About'}]}];}},{label:'Rounded variants — straight to fully rounded',html:`
<cg-stack direction="column" gap="md">
...
`.trim(),setup:(el)=>{const items=[{label:'File',children:[{id:'new',label:'New'},{id:'save',label:'Save'}]},{label:'Edit',children:[{id:'undo',label:'Undo'},{id:'redo',label:'Redo'}]},{label:'View',children:[{id:'zoom',label:'Zoom in'}]}];el.querySelectorAll('cg-menubar').forEach((m)=>{(m as any).items=items;});}},{label:'Size variants — sm / md / lg',html:`
<cg-stack direction="column" gap="md">
...
`.trim(),setup:(el)=>{const items=[{label:'File',children:[{id:'new',label:'New'},{id:'save',label:'Save'}]},{label:'Edit',children:[{id:'undo',label:'Undo'},{id:'redo',label:'Redo'}]},{label:'View',children:[{id:'zoom',label:'Zoom'}]}];el.querySelectorAll('cg-menubar').forEach((m)=>{(m as any).items=items;});}},{label:'Indicator: underline (modern) vs none (canonical)',html:`
<cg-stack direction="column" gap="md">
  <cg-card padding="md">
    <cg-stack direction="column" gap="sm">
...
`.trim(),setup:(el)=>{const items=[{label:'File',children:[{id:'new',label:'New'},{id:'save',label:'Save'}]},{label:'Edit',children:[{id:'undo',label:'Undo'},{id:'redo',label:'Redo'}]},{label:'View',children:[{id:'zoom',label:'Zoom'}]},{label:'Help',children:[{id:'about',label:'About'}]}];el.querySelectorAll('cg-menubar').forEach((m)=>{(m as any).items=items;});}}], since:'v0.8.0' },
  { tag: 'cg-navigation-menu', name: 'Navigation Menu', category: 'navigation', description: 'Mega-menu with hover-reveal panels, sectioned link groups, animated caret, optional icons per link, configurable column count, and keyboard navigation. Triggers have aria-controls linkage; Escape closes; focus rings on triggers and items.', props: [{name:'items',type:'NavMenuItem[]',default:'[]',description:'Top-level menu triggers, each with one or more sections of links'},{name:'columns',type:'1 | 2 | 3 | 4',default:'2',description:'Panel grid column count'},{name:'openDelay',type:'number',default:'80',description:'Hover open delay (ms)'},{name:'closeDelay',type:'number',default:'120',description:'Hover close delay (ms)'}], events: [{name:'cg-navigation-menu-select',detail:'{menu: string, link: string, href?: string}',description:'A link inside a panel was activated'}], examples: [{label:'Two-trigger mega-menu (default)',html:'<cg-navigation-menu id="demo-nav-menu-basic" style="padding-bottom:340px;"></cg-navigation-menu>',setup:(el)=>{const n=el.querySelector("cg-navigation-menu");if(n)n.items=[{label:"Products",sections:[{heading:"Build",links:[{title:"Components",description:"183 ready-to-ship Lit web components.",href:"#"},{title:"Tokens",description:"Tier-3 design tokens with WCAG-validated palettes.",href:"#"},{title:"Templates",description:"Production-ready page scaffolds.",href:"#"}]},{heading:"AI",links:[{title:"Generative UI",description:"Streaming JSON parser + Lit renderer.",href:"#"},{title:"Bias engine",description:"181 cognitive bias cards.",href:"#"}]}]},{label:"Resources",sections:[{links:[{title:"Documentation",description:"Comprehensive guides for every feature.",href:"#"},{title:"Changelog",description:"Release notes and migration guides.",href:"#"},{title:"GitHub",description:"Source on GitHub — star us!",href:"#"}]}]}];}},{label:'With icons per link',html:'<cg-navigation-menu id="demo-nav-menu-icons" style="padding-bottom:380px;"></cg-navigation-menu>',setup:(el)=>{const n=el.querySelector("cg-navigation-menu");const ico=(d)=>`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="${d}"/></svg>`;if(n)n.items=[{label:"Platform",sections:[{links:[{title:"Build",description:"Compose UIs with Lit components.",icon:ico("M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5"),href:"#"},{title:"Theme",description:"Token-driven dark + light themes.",icon:ico("M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"),href:"#"},{title:"Ship",description:"Tree-shakable per-component bundles.",icon:ico("M5 12h14M12 5l7 7-7 7"),href:"#"}]}]},{label:"Learn",sections:[{links:[{title:"Docs",description:"Get up to speed quickly.",icon:ico("M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z M14 2v6h6"),href:"#"},{title:"Examples",description:"Hundreds of working starters.",icon:ico("M16 18l6-6-6-6 M8 6l-6 6 6 6"),href:"#"}]}]}];}},{label:'Single column (compact list)',html:'<cg-navigation-menu id="demo-nav-menu-1col" columns="1" style="padding-bottom:280px;"></cg-navigation-menu>',setup:(el)=>{const n=el.querySelector("cg-navigation-menu");if(n)n.items=[{label:"Solutions",sections:[{links:[{title:"For startups",description:"Get to market fast.",href:"#"},{title:"For enterprises",description:"Audit-ready tokens + a11y.",href:"#"},{title:"For agencies",description:"White-label theming.",href:"#"}]}]}];}},{label:'Three columns (wide panel)',html:'<cg-navigation-menu id="demo-nav-menu-3col" columns="3" style="padding-bottom:340px;"></cg-navigation-menu>',setup:(el)=>{const n=el.querySelector("cg-navigation-menu");if(n)n.items=[{label:"Explore",sections:[{heading:"Foundation",links:[{title:"Identity",description:"Auth + user management.",href:"#"},{title:"Storage",description:"Object + relational.",href:"#"}]},{heading:"Compute",links:[{title:"Edge",description:"Run code at 300+ locations.",href:"#"},{title:"Cron",description:"Scheduled jobs.",href:"#"}]},{heading:"Tools",links:[{title:"CLI",description:"Deploy from terminal.",href:"#"},{title:"Dashboard",description:"Visual control plane.",href:"#"}]}]}];}},{label:'Three triggers, deep panels',html:'<cg-navigation-menu id="demo-nav-menu-rich" style="padding-bottom:360px;"></cg-navigation-menu>',setup:(el)=>{const n=el.querySelector("cg-navigation-menu");if(n)n.items=[{label:"Platform",sections:[{heading:"Foundation",links:[{title:"Identity",description:"Authentication and user management.",href:"#"},{title:"Storage",description:"Object + relational data.",href:"#"},{title:"Edge",description:"Run code at 300+ locations.",href:"#"}]},{heading:"Tools",links:[{title:"CLI",description:"Deploy from your terminal.",href:"#"},{title:"Dashboard",description:"Manage everything visually.",href:"#"}]}]},{label:"Developers",sections:[{links:[{title:"API reference",description:"Full REST + GraphQL docs.",href:"#"},{title:"SDKs",description:"TypeScript, Python, Go, Rust.",href:"#"},{title:"Examples",description:"Hundreds of starter templates.",href:"#"}]}]},{label:"Company",sections:[{links:[{title:"About",href:"#"},{title:"Blog",href:"#"},{title:"Careers",description:"We are hiring.",href:"#"},{title:"Contact",href:"#"}]}]}];}}], since:'v0.8.0' },
  { tag: 'cg-sheet', name: 'Sheet', category: 'overlays', description: 'Slide-in sheet panel with snap points, dismiss gesture, and 4-sided positioning.', props: [{name:'open',type:'boolean',default:'false',description:'Open state'},{name:'label',type:'string',default:'"Sheet"',description:'Accessible label'},{name:'side',type:'"bottom"|"right"|"left"|"top"',default:'"bottom"',description:'Slide direction'},{name:'snapPoints',type:'number[]',description:'Snap positions as viewport fractions'},{name:'activeSnap',type:'number',default:'0',description:'Active snap index'},{name:'dismissible',type:'boolean',default:'true',description:'Click/swipe to dismiss'}], events: [{name:'cg-sheet-close',detail:'{}',description:'Sheet dismissed'}], examples: [{label:'Bottom sheet',html:'<cg-button variant="primary" onclick="this.nextElementSibling.open=true">Open bottom sheet</cg-button><cg-sheet side="bottom" label="Quick actions"><cg-stack direction="column" gap="md"><cg-text size="md" weight="semibold">Choose an action</cg-text><cg-stack direction="column" gap="xs"><cg-button variant="tertiary">Share</cg-button><cg-button variant="tertiary">Edit</cg-button><cg-button variant="tertiary">Move</cg-button><cg-button variant="tertiary">Archive</cg-button></cg-stack></cg-stack></cg-sheet>'},{label:'Right side panel',html:'<cg-button variant="primary" onclick="this.nextElementSibling.open=true">Open from right</cg-button><cg-sheet side="right" label="Filters"><cg-stack direction="column" gap="md"><cg-text size="md" weight="semibold">Filters</cg-text><cg-checkbox label="Active only"></cg-checkbox><cg-checkbox label="Premium tier"></cg-checkbox><cg-checkbox label="With preview"></cg-checkbox><cg-checkbox label="Archived"></cg-checkbox><cg-button variant="primary">Apply</cg-button></cg-stack></cg-sheet>'},{label:'Snap points (40% / 90%)',html:'<cg-button variant="primary" onclick="this.nextElementSibling.open=true">Open with snap points</cg-button><cg-sheet side="bottom" label="Player" .snapPoints=${[0.4,0.9]}><cg-stack direction="column" gap="sm"><cg-text size="md" weight="semibold">Snap-aware sheet</cg-text><cg-text size="sm" muted>Drag the handle to snap between 40% and 90% of viewport height.</cg-text></cg-stack></cg-sheet>'}], since:'v0.8.0' },
  { tag: 'cg-toaster', name: 'Toaster', category: 'overlays', description: 'Viewport-corner toast queue. The toaster element renders nothing on its own — push messages through its imperative `show({ title, description?, variant?, duration?, action? })` method. Auto-dismiss after `duration` ms (set 0 to require manual dismiss).', props: [{name:'position',type:'"top-right" | "top-left" | "bottom-right" | "bottom-left" | "top-center" | "bottom-center"',default:'"bottom-right"',description:'Stack position'},{name:'max',type:'number',default:'5',description:'Max visible toasts'}], events: [{name:'cg-toast-dismiss',detail:'{id: string}',description:'Toast dismissed (auto or manual)'}], examples: [{label:'Live demo — click to push',html:`
...
`.trim(), setup: (el)=>{
  const t = el.querySelector('cg-toaster') as any;
  const messages: Record<string, { title: string; description: string }> = {
    success: { title: 'Saved successfully', description: 'Your changes are live.' },
    error: { title: 'Failed to save', description: 'Network error — please try again.' },
    warning: { title: 'Almost out of space', description: 'You have 12% storage remaining.' },
    info: { title: 'Background sync', description: 'Pulled 3 new updates.' },
    ai: { title: 'AI suggestion', description: 'Try compressing this paragraph.' },
  };
  el.querySelectorAll('cg-button[data-variant]').forEach((btn) => {
    btn.addEventListener('click', () => {
      const v = (btn as HTMLElement).dataset.variant!;
      t?.show({ ...messages[v], variant: v, duration: 4000 });
    });
  });
}},{label:'Pre-stacked (duration: 0 — manual dismiss)',html:`
...
`.trim(), setup: (el)=>{
  ...
  customElements.whenDefined('cg-toaster').then(() => {
    t?.show({ title: 'Welcome back', description: 'You have 3 unread items.', variant: 'info', duration: 0 });
    t?.show({ title: 'Build complete', description: 'Deployed to production.', variant: 'success', duration: 0 });
    t?.show({ title: 'Action required', description: 'Click to confirm.', variant: 'warning', duration: 0 });
  });
}}], since:'v0.8.0' },
  ...
  { tag: 'cg-tree-view', name: 'Tree View', category: 'navigation', description: 'Hierarchical tree with expand/collapse, single or multi-selection, optional leading icons per node, and full WAI-ARIA keyboard navigation. Items are typed as `TreeItem { label, value?, icon?, children?, expanded?, disabled? }` — pass `icon` to render a leading cg-icon next to the label.', props: [{name:'items',type:'TreeItem[]',default:'[]',description:'Hierarchical item tree. Each TreeItem: label (required), optional value, icon (cg-icon name), children, expanded (initial), disabled.'},{name:'multiple',type:'boolean',default:'false',description:'Multi-selection mode — clicking adds/removes from selected instead of replacing.'},{name:'selected',type:'string[]',default:'[]',description:'Selected item values (or paths if no value is set on the item).'}], events: [{name:'cg-tree-select',detail:'{ value: string | string[] }',description:'Selection changed. Single string in single-select; array in multi-select.'},{name:'cg-tree-expand',detail:'{ path: string }',description:'A parent node was expanded.'},{name:'cg-tree-collapse',detail:'{ path: string }',description:'A parent node was collapsed.'}], examples: [{label:'File tree with icons',html:'<cg-tree-view style="max-width:380px;"></cg-tree-view>',setup:(el)=>{const t=el.querySelector('cg-tree-view') as any;if(t)t.items=[{label:'src',icon:'folder',expanded:true,children:[{label:'components',icon:'folder',expanded:true,children:[{label:'cg-button',icon:'code'},{label:'cg-tree-view',icon:'code'},{label:'cg-card',icon:'code'}]},{label:'utils',icon:'folder',children:[{label:'focus-trap.ts',icon:'file'},{label:'roving-index.ts',icon:'file'}]},{label:'index.ts',icon:'file'}]},{label:'tests',icon:'folder',children:[{label:'cg-button.test.ts',icon:'file'},{label:'cg-tree-view.test.ts',icon:'file'}]},{label:'package.json',icon:'document'},{label:'README.md',icon:'document'},{label:'.gitignore',icon:'file',disabled:true}];}},{label:'Multi-select',html:'<cg-tree-view multiple style="max-width:380px;"></cg-tree-view>',setup:(el)=>{const t=el.querySelector('cg-tree-view') as any;if(t){t.items=[{label:'Inbox',icon:'mail',value:'inbox'},{label:'Sent',icon:'send',value:'sent'},{label:'Drafts',icon:'document',value:'drafts'},{label:'Folders',icon:'folder',expanded:true,children:[{label:'Work',value:'work'},{label:'Personal',value:'personal'},{label:'Archive',value:'archive'}]}];t.selected=['inbox','work'];}}},{label:'Plain — no icons (minimalist outline)',html:'<cg-tree-view style="max-width:340px;"></cg-tree-view>',setup:(el)=>{const t=el.querySelector('cg-tree-view') as any;if(t)t.items=[{label:'Introduction',expanded:true,children:[{label:'Why this exists'},{label:'Who it is for'}]},{label:'Getting started',children:[{label:'Installation'},{label:'First example'}]},{label:'Components'},{label:'Tokens'}];}},{label:'Try the keyboard',html:`
<cg-stack direction="column" gap="md">
  <cg-card padding="md">
...
`.trim(),setup:(el)=>{const t=el.querySelector('cg-tree-view') as any;if(t)t.items=[{label:'docs',icon:'folder',expanded:true,children:[{label:'guide.md',icon:'document'},{label:'api.md',icon:'document'}]},{label:'src',icon:'folder',children:[{label:'main.ts',icon:'code'}]},{label:'README.md',icon:'document'}];}}], since:'v0.8.0' },
  { tag: 'cg-portal', name: 'Portal', category: 'layout', description: 'Headless transport primitive. Teleports its slotted children into a target DOM node (default: document.body) — escapes clipping ancestors and stacking contexts. Used internally by modal, popover, tooltip, sheet, and toaster. Set `disabled` to render inline (SSR / no-portal mode).', props: [{name:'target',type:'string | HTMLElement',default:'""',description:'CSS selector or HTMLElement reference. Empty string falls back to document.body.'},{name:'disabled',type:'boolean',default:'false',description:'When true, children render in place instead of teleporting'}], events: [{name:'cg-portal-mount',detail:'{ container: HTMLElement }',description:'Children have been teleported to the target'},{name:'cg-portal-unmount',detail:'none',description:'Children have been returned to the host (target change, disable, or disconnect)'}], examples: [{label:'Inline (disabled)',html:`
<cg-portal disabled>
...
`.trim()},{label:'Default (teleports to document.body)',html:`
...
`.trim()},{label:'Modal pattern (used internally by cg-modal)',html:`
<cg-portal disabled>
...
`.trim()}], since:'v0.8.0' },
  { tag: 'cg-focus-scope', name: 'Focus Scope', category: 'layout', description: 'Headless focus-trap primitive. Cycles Tab/Shift+Tab within its children when active, and (optionally) returns focus to the previously-focused element on deactivate. Renders no chrome — uses display:contents — so wrap with cg-card / cg-stack / your own surface to make it visible.', props: [{name:'active',type:'boolean',default:'false',description:'Trap is active'},{name:'returnFocus',type:'boolean',default:'true',description:'Restore previous focus on deactivate'}], events: [], examples: [{label:'Active trap inside a card',html:`
<cg-card padding="lg">
...
`.trim()},{label:'Inactive (Tab leaves freely)',html:`
<cg-card padding="lg">
  <cg-focus-scope>
    <cg-stack direction="column" gap="md">
...
`.trim()},{label:'Form trap (modal pattern)',html:`
<cg-card padding="lg">
  <cg-focus-scope active>
    <cg-stack direction="column" gap="md">
...
`.trim()}], since:'v0.8.0' },
  { tag: 'cg-visually-hidden', name: 'Visually Hidden', category: 'layout', description: 'Accessibility helper — renders children for assistive technology but hides them visually using the canonical W3C / a11y-project recipe (1×1px clipped box, taken out of flow). Use for icon-only buttons, ambiguous links, sortable column states, and other places where extra context is needed for screen readers but not for sighted users.', props: [], events: [], examples: [{label:'Icon-only button — accessible name',html:`
<cg-stack direction="column" gap="md">
...
`.trim()},{label:'Ambiguous link — extended context',html:`
<cg-stack direction="column" gap="md">
...
`.trim()},{label:'Sortable column header — announces sort state',html:`
<cg-stack direction="column" gap="md">

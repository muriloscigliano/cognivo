...
`.trim(),setup:(el)=>{const c=el.querySelector('cg-command') as any;const btn=el.querySelector('#cmd-empty-trigger');if(c)c.commands=[{id:'a',label:'Apple',group:'Fruit'},{id:'b',label:'Banana',group:'Fruit'}];btn?.addEventListener('click',()=>{if(c)c.open=true;});}}], since:'v0.7.0' },
  ...
  { tag: 'cg-blockquote', name: 'Blockquote', category: 'typography', description: 'Styled quotation primitive rendering a semantic <blockquote> with an accent border and optional attribution footer.', props: [{name:'cite',type:'string',default:'""',description:'Source URL (blockquote cite)'},{name:'variant',type:'"default" | "accent" | "muted"',default:'"default"',description:'Border/background emphasis'}], events: [], examples: [{label:'Accent',html:'<cg-blockquote variant="accent">Design tokens are the contract between design and code.<span slot="footer">— Cognivo docs</span></cg-blockquote>'},{label:'Muted',html:'<cg-blockquote variant="muted">Think in systems, not screens.</cg-blockquote>'},{label:'Default',html:'<cg-blockquote>Every spacing value should come from the token scale.</cg-blockquote>'}], since:'v0.8.0' },

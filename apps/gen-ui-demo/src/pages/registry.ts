...
export const registry: ComponentMeta[] = [
  ...
  // OVERLAYS
  // ═══════════════════════════════════════════════════════════════════════════
  { tag: 'cg-dropdown', name: 'Dropdown', category: 'overlays', description: 'Floating dropdown menu with scale+fade animation, arrow key navigation, click-outside close.', props: [{name:'items',type:'DropdownItem[]',description:'Menu items'},{name:'open',type:'boolean',default:'false',description:'Open state'},{name:'position',type:'string',default:'"bottom-start"',description:'Position'}], events: [{name:'cg-dropdown-select',detail:'{id, label}',description:'Item selected'}], examples: [{label:'Basic',html:'<cg-dropdown></cg-dropdown>',setup:(el)=>{const d=el.querySelector('cg-dropdown') as any;if(d)d.items=[{id:'edit',label:'Edit'},{id:'copy',label:'Copy'},{id:'delete',label:'Delete',divider:true}];}}], since:'v0.5.0' },
  { tag: 'cg-modal', name: 'Modal', category: 'overlays', description: 'Modal dialog with backdrop blur, spring-bounce scale animation, focus trap, body scroll lock.', props: [{name:'open',type:'boolean',default:'false',description:'Open state'},{name:'title',type:'string',description:'Modal title'},{name:'size',type:'"sm"|"md"|"lg"|"xl"',default:'"md"',description:'Size'}], events: [{name:'cg-modal-close',detail:'{}',description:'Close requested'}], examples: [{label:'Demo',html:'<cg-button onclick="this.nextElementSibling.open=true">Open Modal</cg-button><cg-modal title="Confirm Action"><p style="color:var(--text-secondary)">Are you sure you want to proceed?</p></cg-modal>'}], since:'v0.5.0' },
  { tag: 'cg-tooltip', name: 'Tooltip', category: 'overlays', description: 'Hover/focus tooltip with CSS arrow, configurable delay, 4 positions.', props: [{name:'content',type:'string',description:'Tooltip text'},{name:'position',type:'"top"|"bottom"|"left"|"right"',default:'"top"',description:'Position'},{name:'delay',type:'number',default:'300',description:'Show delay (ms)'}], events: [], examples: [{label:'Positions',html:'<cg-stack direction="row" gap="lg"><cg-tooltip content="Top tooltip" position="top"><cg-button>Top</cg-button></cg-tooltip><cg-tooltip content="Bottom tooltip" position="bottom"><cg-button>Bottom</cg-button></cg-tooltip></cg-stack>'}], since:'v0.5.0' },
  { tag: 'cg-drawer', name: 'Drawer', category: 'overlays', description: 'Slide-in side panel with spring-bounce animation, backdrop, 4 sizes.', props: [{name:'open',type:'boolean',default:'false',description:'Open state'},{name:'side',type:'"left"|"right"',default:'"right"',description:'Slide from side'},{name:'title',type:'string',description:'Drawer title'},{name:'size',type:'"sm"|"md"|"lg"|"full"',default:'"md"',description:'Width'}], events: [{name:'cg-drawer-close',detail:'{}',description:'Close requested'}], examples: [{label:'Demo',html:'<cg-button onclick="this.nextElementSibling.open=true">Open Drawer</cg-button><cg-drawer title="Settings" side="right"><p style="color:var(--text-secondary);padding:16px;">Drawer content here</p></cg-drawer>'}], since:'v0.5.0' },

  // ═══════════════════════════════════════════════════════════════════════════
  // FEEDBACK
  // ═══════════════════════════════════════════════════════════════════════════
  { tag: 'cg-progress-bar', name: 'Progress Bar', category: 'feedback', description: 'Linear progress bar with smooth transitions, indeterminate mode, striped pattern, 4 color variants.', props: [{name:'value',type:'number',default:'0',description:'Progress 0-100'},{name:'variant',type:'"default"|"success"|"warning"|"danger"',default:'"default"',description:'Color variant'},{name:'indeterminate',type:'boolean',default:'false',description:'Indeterminate mode'},{name:'striped',type:'boolean',default:'false',description:'Striped pattern'}], events: [], examples: [{label:'Variants',html:'<cg-stack gap="md" style="max-width:400px;"><cg-progress-bar value="75" label="Upload" showValue></cg-progress-bar><cg-progress-bar value="40" variant="warning" striped animated></cg-progress-bar><cg-progress-bar indeterminate variant="success"></cg-progress-bar></cg-stack>'}], since:'v0.5.0' },
  ...
  { tag: 'cg-skeleton', name: 'Skeleton', category: 'feedback', description: 'Loading placeholder with shimmer animation. Text, circular, and rectangular variants.', props: [{name:'variant',type:'"text"|"circular"|"rectangular"',default:'"text"',description:'Shape variant'},{name:'lines',type:'number',default:'3',description:'Text lines count'},{name:'width',type:'string',description:'Width CSS'},{name:'height',type:'string',description:'Height CSS'}], events: [], examples: [{label:'Variants',html:'<cg-stack gap="md" style="max-width:300px;"><cg-skeleton variant="text" lines="3"></cg-skeleton><cg-skeleton variant="circular" width="48px" height="48px"></cg-skeleton><cg-skeleton variant="rectangular" width="100%" height="120px"></cg-skeleton></cg-stack>'}], since:'v0.5.0' },

  // ═══════════════════════════════════════════════════════════════════════════
  // FOUNDATION EXTRAS
  // ═══════════════════════════════════════════════════════════════════════════
  { tag: 'cg-breadcrumbs', name: 'Breadcrumbs', category: 'foundation', description: 'Navigation breadcrumb trail with separator and icon support.', props: [{name:'items',type:'{label, href?, icon?}[]',description:'Breadcrumb items'},{name:'separator',type:'string',default:'"/"',description:'Separator character'}], events: [{name:'cg-breadcrumb-click',detail:'{item}',description:'Item clicked'}], examples: [{label:'Basic',html:'<cg-breadcrumbs></cg-breadcrumbs>',setup:(el)=>{const b=el.querySelector('cg-breadcrumbs') as any;if(b)b.items=[{label:'Home',href:'#'},{label:'Components',href:'#'},{label:'Breadcrumbs'}];}}], since:'v0.5.0' },
  { tag: 'cg-pagination', name: 'Pagination', category: 'data', description: 'Page navigation with prev/next arrows, ellipsis gaps, and accent-highlighted current page.', props: [{name:'total',type:'number',description:'Total pages'},{name:'current',type:'number',default:'1',description:'Current page'},{name:'siblings',type:'number',default:'1',description:'Pages around current'}], events: [{name:'cg-page-change',detail:'{page}',description:'Page changed'}], examples: [{label:'Basic',html:'<cg-pagination total="20" current="5"></cg-pagination>'}], since:'v0.5.0' },
  { tag: 'cg-chip', name: 'Chip', category: 'foundation', description: 'Removable pill tag with 5 color variants, press scale animation, keyboard delete.', props: [{name:'label',type:'string',description:'Chip text'},{name:'variant',type:'"default"|"success"|"warning"|"error"|"accent"',default:'"default"',description:'Color'},{name:'removable',type:'boolean',default:'false',description:'Show X button'},{name:'size',type:'"sm"|"md"',default:'"md"',description:'Size'}], events: [{name:'cg-chip-remove',detail:'{label}',description:'Remove clicked'}], examples: [{label:'Variants',html:'<cg-stack direction="row" gap="sm"><cg-chip label="Default"></cg-chip><cg-chip label="Success" variant="success"></cg-chip><cg-chip label="Warning" variant="warning" removable></cg-chip><cg-chip label="Error" variant="error" removable></cg-chip><cg-chip label="Accent" variant="accent"></cg-chip></cg-stack>'}], since:'v0.5.0' },
  ...
  // FORM EXTRAS
  // ═══════════════════════════════════════════════════════════════════════════
  { tag: 'cg-number-input', name: 'Number Input', category: 'forms', description: 'Number input with +/- increment buttons, long-press repeat, keyboard arrows.', props: [{name:'value',type:'number',default:'0',description:'Current value'},{name:'min',type:'number',description:'Minimum'},{name:'max',type:'number',description:'Maximum'},{name:'step',type:'number',default:'1',description:'Step increment'}], events: [{name:'cg-change',detail:'{value}',description:'Value changed'}], examples: [{label:'Basic',html:'<cg-number-input value="5" min="0" max="100" label="Quantity" style="max-width:200px;"></cg-number-input>'}], since:'v0.5.0' },
  { tag: 'cg-otp-input', name: 'OTP Input', category: 'forms', description: 'One-time password input with individual digit boxes, auto-advance, paste support.', props: [{name:'length',type:'number',default:'6',description:'Number of digits'},{name:'mask',type:'boolean',default:'false',description:'Show dots instead of digits'},{name:'error',type:'boolean',default:'false',description:'Error state'}], events: [{name:'cg-otp-complete',detail:'{value}',description:'All digits entered'}], examples: [{label:'Default',html:'<cg-otp-input length="6"></cg-otp-input>'},{label:'Masked',html:'<cg-otp-input length="4" mask></cg-otp-input>'}], since:'v0.5.0' },
  { tag: 'cg-autocomplete', name: 'Autocomplete', category: 'forms', description: 'Combobox input with filtered dropdown suggestions, arrow key navigation, highlight matching.', props: [{name:'options',type:'{value, label}[]',description:'Options list'},{name:'placeholder',type:'string',description:'Placeholder'},{name:'clearable',type:'boolean',default:'false',description:'Show clear button'}], events: [{name:'cg-autocomplete-select',detail:'{value, label}',description:'Option selected'}], examples: [{label:'Basic',html:'<cg-autocomplete placeholder="Search countries..." clearable style="max-width:300px;"></cg-autocomplete>',setup:(el)=>{const a=el.querySelector('cg-autocomplete') as any;if(a)a.options=[{value:'us',label:'United States'},{value:'uk',label:'United Kingdom'},{value:'ca',label:'Canada'},{value:'au',label:'Australia'},{value:'de',label:'Germany'},{value:'fr',label:'France'},{value:'jp',label:'Japan'},{value:'br',label:'Brazil'}];}}], since:'v0.5.0' },
  ...
  { tag: 'ai-empty-state', name: 'Empty State', category: 'ai-collab', description: 'Contextual empty state with icon, title, description, and action button.', props: [{name:'icon',type:'string',description:'Emoji icon'},{name:'title',type:'string',description:'Title'},{name:'description',type:'string',description:'Description'},{name:'actionLabel',type:'string',description:'Button label'}], events: [{name:'ai-empty-action',detail:'{}',description:'Action clicked'}], examples: [{label:'No data',html:'<ai-empty-state icon="📊" title="No data yet" description="Connect a data source to start analyzing." actionLabel="Connect Data" style="max-width:350px;"></ai-empty-state>'}], since:'v0.5.0' },
